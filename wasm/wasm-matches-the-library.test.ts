...
import { CALCULATOR_BUILDERS, findCalculatorBuilder } from '../src/builders/index'
import { LAB_CATALOG } from '../src/data/index'
import { formatGroupedNumber, formatNumberForDisplay } from '../src/formatting/index'
...
const MODULE_PATH = path.join(HERE, '.build', 'thetowersdk.wasm')
...
async function loadCaller(): Promise<Call> {
  const module = (await import('./run.mjs')) as { call: Call }
  return module.call
}
...
describe.skipIf(!built)('the WASM module answers as the library does', () => {
  ...
  it('decodes a real save to the same account the library reads', async () => {
     ...
     * file at all.
     */
    const fixture = path.resolve(HERE, '../../../android/app/src/main/assets/public/playerInfo.dat')
    ...
    const { decodeInflatedSave } = await import('../src/save-decoder/index')
    const { listImportableBattleRuns } = await import('../src/save/index')
