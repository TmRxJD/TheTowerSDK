...
import {
  ...
  lookupGlossary,
} from '../../src/data/glossary/glossary'
...
function collectCatalogNames(): Set<string> {
  ...
  for (const type of (catalog.MODULE_TYPE_ROWS as CatalogRow[]) ?? []) push(type.name)
...
describe('glossary', () => {
  ...
  it('has entries', () => {
    expect(GLOSSARY_NAME_ROWS.length).toBeGreaterThan(100)
