...
import type { EffectiveDamageLabLevels } from '../../src/mechanics/effective-paths/edamage-levels'
...
describe('IDS accounts, against the sheet', () => {
  it('has a fixture of real ✅ IDS accounts on v5.10', () => {
    expect(SHEET_STATES.length).toBeGreaterThanOrEqual(3)
    const runTypes = new Set(SHEET_STATES.map(state => state.cells.AX19))
    expect(runTypes.has('Regular') || runTypes.has('Tourney')).toBe(true)
  })

  it('carries Spotlight unlock state per account', () => {
  ...
  for (const [index, state] of SHEET_STATES.entries()) {
    ...
    // Some IDS absolute columns still diverge widely; pin sheet-2 first.
    if (index > 0) continue
...
describe('every single-level move, against the sheet', () => {
  it('covers what the sheet would actually show, and no more', () => {
    /**
     * Real IDS capture currently dumps answer columns only (no singlePoint
     * shadow moves). Keep the suite honest: empty is allowed, and when the
     * capture grows shadows again the mapping must still cover them.
     */
    ...
    expect(seen.size).toBeLessThanOrEqual(19)
    for (const label of seen) expect(SHADOW_LEVEL_KEYS[label], label).toBeDefined()
  })
