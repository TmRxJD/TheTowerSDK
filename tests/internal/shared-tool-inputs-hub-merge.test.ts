...
import { getSharedToolInputs, mergeEnemyStatsCoreHub, mergeSharedUptimeInputsHub } from '../../src/internal/shared-tool-inputs/shared-tool-inputs'
import { compactSharedToolInputsForStorage } from '../../src/internal/shared-tool-inputs/storage'
import { defaultSharedEnemyStatsCore } from '../../src/internal/shared-tool-inputs/extended'
...
describe('shared-tool-inputs hub merge', () => {
  ...
  it('stores sparse hub values and resolves defaults at read time', () => {
    const resolved = getSharedToolInputs({
    ...
    const reloaded = getSharedToolInputs(sparse)
  ...
  it('does not let uptime Math.max clobber lower hub wavesPerBoss on delta merge', () => {
    const existing = getSharedToolInputs({ uptimeInputs: { wavesPerBoss: 3 } }).uptimeInputs
    const incoming = getSharedToolInputs({ uptimeInputs: { wavesPerBoss: 10 } }).uptimeInputs
