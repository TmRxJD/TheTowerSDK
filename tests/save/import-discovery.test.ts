...
} from '../../src/save/bots/read'
import { CARDS_SAVE_LEVEL_KEY, CARDS_SAVE_UNLOCKED_KEY } from '../../src/save/cards/read'
import { planBattleReportImport } from '../../src/save/battle/reports'
import { discoverSaveImportTargets } from '../../src/save/import/discovery'
import { countImportableItemsFromSaveRoot } from '../../src/save/import/counts'
...
describe('discoverSaveImportTargets', () => {
  it('includes battle report and lifetime summaries', () => {
    const result = discoverSaveImportTargets({
      battleHistory: {
        _items: [{
          tier: 21,
          wave: 4695,
    ...
    const battle = result.targets.find(tracker => tracker.key === 'battleReports')
    const lifetime = result.targets.find(tracker => tracker.key === 'lifetime')
  ...
  it('discovers bots and cards using the same extractors as the website import counts', () => {
    ...
    const result = discoverSaveImportTargets(parsedRoot)
    const bots = result.targets.find(tracker => tracker.key === 'bots')
    const cards = result.targets.find(tracker => tracker.key === 'cards')
