...
import { executeSaveImportTarget, executeSaveImports } from '../../src/save/import/executor'
import { planSaveImportTarget } from '../../src/save/import/planner'
...
/**
 * The step that actually writes a save into a player's targets.
 *
 ...
 * twelve targets in a loop, and a player who runs it has already committed:
 * there is no undo.
 *
...
describe('a tracker the planner turned down', () => {
  it('is never written, whatever the caller does with it', async () => {
    ...
    for (const key of KEYS) {
      const planner = planSaveImportTarget(key, {})
      const outcome = await executeSaveImportTarget(planner, {}, port)
  ...
  it('carries the planner’s reason rather than a generic one', async () => {
    ...
    const planner = planSaveImportTarget('workshop', {})
    const outcome = await executeSaveImportTarget(planner, {}, port)
...
describeSave('writing a real save', () => {
  ...
  it('keeps going when one tracker throws', async () => {
    /*
     * The property a partial import depends on. Twelve targets are written in
  ...
  it('writes nothing for a selection of nothing', async () => {
    const { written, port } = recordingPort()
    const outcomes = await executeSaveImports({ parsedRoot, selectedKeys: [], port })
  ...
  it('writes only what was selected', async () => {
    ...
    // its neighbours.
    const { written, port } = recordingPort()
    await executeSaveImports({ parsedRoot, selectedKeys: ['labs'], port })
  ...
  it('does not mutate the save root while writing', async () => {
    ...
    await executeSaveImports({ parsedRoot, selectedKeys: KEYS, port })
  ...
  it('stops rather than half-writing when the caller’s progress hook throws', async () => {
     ...
     * loop. That is a real partial write — some targets persisted, the rest
    ...
    await expect(executeSaveImports({
