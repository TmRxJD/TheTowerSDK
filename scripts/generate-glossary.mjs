#!/usr/bin/env node
/* ── file map ─────────────────────────────────────────────────────────────────
 * exports    GLOSSARY_NAME_ROWS
 * depends    glossary-types
 * used by    —
 * ─────────────────────────────────────────────── generated by `npm run map` ──
 */
/**
 * Regenerates `src/data/generated/glossary-names.generated.ts` from the catalogs.
 *
...
export const GLOSSARY_NAME_ROWS: readonly GlossaryEntry[] = ${JSON.stringify(unique, null, 2)} as const
`

await fs.writeFile(path.join(ROOT, 'src', 'data', 'generated', 'glossary-names.generated.ts'), body, 'utf8')

console.log(
  `wrote src/data/generated/glossary-names.generated.ts — ${unique.length} entries, `
