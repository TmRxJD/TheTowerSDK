...
/**
 * The date this graph is scored against: the newest verification in it.
 *
 * `compartmentMaturity` requires `asOf` and refuses to default it, for a reason it states in its
 * own docblock — "a metric that changes when nothing changed cannot be tested, diffed, or trusted
 * in a report". This script then passed it `new Date()`, which defeated exactly that, under a
 * comment claiming reruns were byte-identical. They were not: the emitted graph ships in `dist`, so
 * two publishes on different days differed with no change to the knowledge behind them.
 *
 * Reading the graph's own horizon instead is both reproducible and the more useful comparison —
 * staleness relative to the freshest thing anyone has verified, rather than relative to how long
 * the repository has sat. `ACS_ASOF` still overrides it for a report as of some other date.
 */
function newestVerification(graph) {
  let newest = '1970-01-01'
  const consider = source => {
    if (source?.verifiedAt && source.verifiedAt > newest) newest = source.verifiedAt
  }
  for (const compartment of graph.compartments) {
    for (const node of compartment.nodes) {
      for (const source of node.sources ?? []) consider(source)
      for (const assertion of node.assertions ?? []) consider(assertion.provenance)
    }
  }
  return newest
}

const ASOF = process.env.ACS_ASOF ?? newestVerification(knowledge.GAME_KNOWLEDGE)
