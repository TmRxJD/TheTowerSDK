/* ── file map ─────────────────────────────────────────────────────────────────
 * exports    BotLabInfoRow, BotStatDefinition, BotValueRow, BotStatRow, BotUpgradeTierData, BotPlusData, BotData, BotLabLevels (+51 more)
 * depends    src/mechanics/bot-hit-multiplier, src/mechanics/uptime-core
 * used by    src/data/index, src/internal/bot-lab-levels-from-research, src/internal/bot-medal-splitter-local-state, src/internal/bots-calcs-local-state, src/internal/shared-uptime-inputs, src/knowledge/compartments/bots (+9 more)
...
export function botStatMaxLevel(bot: BotData, statName: string, tier: BotUpgradeTier = 'base'): number {
  const tierData = getTierData(bot, tier)
  const levels = tierData?.stats?.[statName]?.levels
  if (!levels) return 0

  return Object.keys(levels).reduce((best, key) => {
    const level = Number(key)
    return Number.isFinite(level) && level > best ? level : best
  }, 0)
}
