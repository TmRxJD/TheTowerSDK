/* ── file map ─────────────────────────────────────────────────────────────────
 * exports    ToolLabLevel, ToolLabRecord, ToolLabMenuOption, LabProgressModifiers, LabProgressRow, TOOL_LAB_OVERVIEW_CATEGORY_ORDER, TOOL_LAB_SPEEDUP_OPTIONS, isToolLabOverviewCategoryLabel (+19 more)
 * depends    src/data/labs/catalog, src/data/labs/categories, src/data/labs/normalize-category, src/data/labs/research, src/formatting
 * used by    src/data/labs/categories, src/data/labs/compute, src/data/labs/display-overrides, src/data/labs/index, src/internal/persistence/labs-persistence, src/mechanics/calculators/run (+6 more)
...
import { LAB_CATALOG, type LabCatalogRecord } from './catalog'
import {
...
import { normalizeToolLabCategory } from './normalize-category'
export { normalizeToolLabCategory }
import { findLabCategoryForSaveIndex, CANONICAL_LAB_SLUG_BY_ALIAS } from './categories'
...
export function isLabsResearchLabName(name: string | null | undefined): boolean {
  const trimmed = String(name || '').trim()
  if (!trimmed) return false
...
export function compareToolLabSaveKeysForOverview(leftKey: string, rightKey: string): number {
  ...
  const leftCategory = leftResearch
    ? (findLabCategoryForSaveIndex(leftResearch.index) ?? leftResearch.category)
    : null
  const rightCategory = rightResearch
    ? (findLabCategoryForSaveIndex(rightResearch.index) ?? rightResearch.category)
    : null
...
export function findToolLabBySlug(slug: string): ToolLabRecord | undefined {
  ...
  const canonical = Object.prototype.hasOwnProperty.call(CANONICAL_LAB_SLUG_BY_ALIAS, slug)
    ? CANONICAL_LAB_SLUG_BY_ALIAS[slug]!
    : slug
...
export function getLabsByCategory(
  ...
  return labs
    .filter(lab => isLabsResearchLabName(lab.name) || isLabsResearchLabName(lab.displayName))
