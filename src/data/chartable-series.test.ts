...
import { buildGuardianDefinitions } from './guardians'
import { getWorkshopStatDefinitions } from './workshop-tracker-definitions'
...
function countByCategory(): Record<string, number> {
  ...
  for (const bot of BOT_UPGRADES_DATA) {
    for (const stat of Object.values(bot.stats ?? {})) bots += seriesInProgression(stat?.levels)
    for (const stat of Object.values(bot.plus?.stats ?? {})) bots += seriesInProgression(stat?.levels)
  }
