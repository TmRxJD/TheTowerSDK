...
const COMPARTMENTS_DIR = join(__dirname, 'compartments')
...
/**
 * This file is excluded from its own corpus, by ROLE rather than by path.
 *
 * A checker that examines a corpus is inside that corpus, and naming a constant
 * in order to assert something about it is not wiring it. Leaving this file in
 * gave every name mentioned here a phantom reference — including the sentinel
 * that is supposed to be absent, which failed the counter's own self-check.
 *
 * Role, not path, because a path exemption silently stops covering whatever
 * moves into it later.
 */
const CHECKER_FILE = join(__dirname, 'compartment-constants-are-wired.test.ts')
const corpus = [...sources.entries()]
  .filter(([path]) => path !== CHECKER_FILE)
  .map(([, body]) => body)
...
describe('every compartment constant is wired into the graph', () => {
  ...
  it('counts identifiers correctly, so a broken counter cannot report a clean sweep', () => {
    ...
    // And the exclusion above is real: this file mentions PERK_APPLIED_AS too.
    expect(corpus.length).toBe(sources.size - 1)
