...
import { RELIC_TEMPLATES, RELIC_UNLOCK_METHODS } from '../data/relics'
...
type CatalogRow = { name: string, benefit: number, benefitType: number, description?: string, unlockDescription?: string }
...
describe('the catalog against the game enum', () => {
  it('has one template per catalog row, matched by name', () => {
    expect(CATALOG).toHaveLength(RELIC_TEMPLATES.length)
    const names = new Set(RELIC_TEMPLATES.map(relic => relic.name))
    for (const row of CATALOG) expect(names, row.name).toContain(row.name)
  })

  it('maps every benefitType onto exactly one bonusType, and uses all 27', () => {
    const byName = new Map(RELIC_TEMPLATES.map(relic => [relic.name, relic]))
    ...
    for (const row of CATALOG) {
      ...
      const bonusType = byName.get(row.name)?.bonusType ?? '<none>'
  ...
  it('agrees on every value, with the catalog holding the raw fraction', () => {
    const byName = new Map(RELIC_TEMPLATES.map(relic => [relic.name, relic]))
    for (const row of CATALOG) {
      const template = byName.get(row.name)!
...
describe('the rarity label', () => {
  ...
  it('affects a real number of relics, not one stray row', () => {
    ...
    expect(affected.length).toBe(17)
  })
})

describe('the wiki totals exceed the catalog for some stats', () => {
  it('is a real excess, measured against the current templates', () => {
    ...
    expect(RELIC_STATS_WHERE_WIKI_EXCEEDS_CATALOG.length).toBe(6)
  })
})
