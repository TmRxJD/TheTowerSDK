 ...
 */
/* ── file map ─────────────────────────────────────────────────────────────────
 * exports    DEFAULT_STALENESS_DAYS, scoreCompartment, scoreAll, describeMaturity
 * depends    src/knowledge/substrate/contradictions, src/knowledge/substrate/graph, src/knowledge/substrate/schema
 * used by    src/knowledge/substrate/index, src/mechanics/calculators/specs-generated
...
import { withDefaults } from './graph'
...
export function scoreCompartment(
  ...
  /* Through the same defaulting as every other reader, so one node cannot score two ways. */
  const nodes = compartment.nodes.map(withDefaults)
