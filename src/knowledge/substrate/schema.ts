 ...
 */
/* ── file map ─────────────────────────────────────────────────────────────────
 * exports    ClaimOrigin, PRIMARY_ORIGINS, ORIGIN_AUTHORITY, Provenance, ClaimType, VerificationState, Assertion, KnowledgeEdgeKind (+5 more)
 * depends    —
 * used by    src/knowledge/compartments/attack, src/knowledge/compartments/bots, src/knowledge/compartments/build-targets, src/knowledge/compartments/cards, src/knowledge/compartments/combat, src/knowledge/compartments/daily-missions (+29 more)
...
export interface Provenance {
  ...
  verifiedAt: string
  /**
   * Why this source, or which of several — prose, and never part of the identifier.
   *
   * `ref` is machine-read: the wiki drift check fetches it as a page title. Five sources had a
   * qualifier welded into it instead — `'Bounce Shot (game-vault.net — Fandom page is an empty
   * stub)'` — so the checker asked the wiki for a page by that whole sentence, got a 404, and
   * reported "wiki page not found (fix coverage title)" against a page that exists perfectly well.
   *
   * The qualifier is real information and worth keeping; it just cannot live in the field that
   * names the thing. Use `section` when the point is *where on the page*, and this when the point
   * is *which source, and why that one*.
   */
  note?: string
}
