 ...
 */
/* ── file map ─────────────────────────────────────────────────────────────────
 * exports    withDefaults, allNodes, allEdges, nodeById, compartmentOf, relationsOf, neighboursOf, trapsFor (+7 more)
 * depends    src/knowledge/substrate/schema
 * used by    src/knowledge/substrate/index, src/knowledge/substrate/maturity, src/mechanics/calculators/specs-generated
...
/**
 * The defaults the schema documents, applied.
 *
 * `claimType` and `verification` are optional on a node and default to `objective` and
 * `verified_here`. That default lived in two places — the doc comment on the type, and the
 * emitter that writes the shipped JSON — and in neither of the places a caller reads from.
 *
 * So the same graph had two shapes. Read the emitted `knowledge-graph.v1.json` and every node
 * carries both fields; read it through this API and 91 nodes carried neither, so
 * `node.claimType === 'objective'` silently dropped a third of the graph. Both answers came from
 * the same content, and nothing reported the difference.
 *
 * Applying it here means one implementation of the rule, at the surface every consumer uses.
 */
export function withDefaults(node: KnowledgeNode): KnowledgeNode {
  if (node.claimType !== undefined && node.verification !== undefined) return node
  return {
    ...node,
...
/** Every node across every compartment, with the schema's defaults applied. */
export function allNodes(graph: KnowledgeGraph): readonly KnowledgeNode[] {
  return graph.compartments.flatMap(compartment => compartment.nodes.map(withDefaults))
}
