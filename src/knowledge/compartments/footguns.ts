 ...
 */
/* ── file map ─────────────────────────────────────────────────────────────────
 * exports    IRREVERSIBLE_UPGRADES, REVERSIBLE_UPGRADES, COOLDOWN_SYNC_TARGETS, BOT_COOLDOWN_FLOOR_SECONDS, BOT_COOLDOWN_OUTLIER, FOOTGUN_KNOWLEDGE_NODES, FOOTGUN_KNOWLEDGE_EDGES
 * depends    src/data/bots/data, src/data/ultimate-weapons/stones, src/knowledge/substrate/schema
 * used by    src/knowledge/compartments/index, src/knowledge/index
 * ─────────────────────────────────────────────── generated by `npm run map` ──
 */
import { BOT_UPGRADES_DATA, botStatValueAt, type BotStatDefinition } from '../../data/bots/data'
...
type BotCooldownTable = {
  ...
  readonly stats: Readonly<Record<string, BotStatDefinition | undefined>>
...
export const BOT_COOLDOWN_FLOOR_SECONDS: Readonly<Record<string, number>> = Object.fromEntries(
  (BOT_UPGRADES_DATA as unknown as readonly BotCooldownTable[]).map(bot => {
    // The stat's own floor, then its lab. Both are needed: the ladder alone reaches neither
    // published figure.
    const stat = bot.stats.Cooldown
    const ladderFloor = stat ? botStatValueAt(stat, stat.maxLevel) : 0
    const lab = bot.labInfo.find(entry => entry.name === 'Cooldown')
    return [bot.name, ladderFloor + seconds(lab?.maxValue)]
  }),
)
