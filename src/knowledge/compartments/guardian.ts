 ...
 * depends    src/data/guardians/upgrades, src/data/player-stats/data, src/knowledge/substrate/schema, src/support/own-lookup
...
import { GUARDIAN_CHIP_TYPE_ENUM } from '../../data/player-stats/data'
 ...
 * `targetKey` — so a lookup by display name resolved to the dead entry and read
...
export const GUARDIAN_KNOWLEDGE_NODES: readonly KnowledgeNode[] = [
  ...
  },
  {
    ...
    traps: [
      'THE CHIP KEYS USED TO SIT ON THE WRONG TWIN. `attack` was on `Scare`, `ally` on `Rush`, '
      ...
      'THREE CHIP TYPES STILL HAVE NO KEY OF THEIR OWN — Catch, Scare and Rush. They are real ChipType '
      + 'values with real behaviour in `TowerBuddy`, and they are not the player-facing chips. A '
      + 'null key means "not a player-facing chip", not "missing data".',
    ...
    assertions: [
      ...
      { subject: 'guardian.chip', predicate: 'nullKeyCount', value: CHIP_ENTRIES.filter(c => c.targetKey === null).length, provenance: CATALOG_GUARDIAN },
...
export const GUARDIAN_KNOWLEDGE_EDGES: readonly KnowledgeEdge[] = [
  ...
  },
  {
    ...
    note:
      ...
      + 'three display labels are ambiguous and lookup-by-label returns the null-targetKey entry.',
    sources: [CATALOG_GUARDIAN],
  },
]
