 ...
 */
/* ── file map ─────────────────────────────────────────────────────────────────
 * exports    BOT_UPGRADE_STATS_BY_BOT, BOT_UNIVERSAL_UPGRADE_STATS, BOT_UPGRADE_STAT_UNION, BOT_UPGRADE_BASE_MEDAL_COST, BOT_UPGRADE_MEDAL_COST_STEP, BOT_PLUS_STONE_COST, BOT_STAT_MAX_LEVEL, BOT_BASE_COST_LADDER (+6 more)
 * depends    src/data/bots/data, src/knowledge/substrate/schema
 * used by    src/knowledge/compartments/index, src/knowledge/index
 * ─────────────────────────────────────────────── generated by `npm run map` ──
 */
import { BOT_BASE_UNLOCK_COSTS, BOT_UPGRADES_DATA, botStatMaxLevel, type BotData , type BotStatDefinition} from '../../data/bots/data'
...
type BotStatTable = BotStatDefinition
type BotTierTable = {
...
/**
 * Every stat's own ceiling, read through the data layer's own accessor.
 *
 * This computed the ceiling itself, from the same tables, in a second implementation. That was
 * harmless right up until the bot calculator needed the same number and read the shared cost
 * ladder instead -- selling 15 levels of Flame Bot Cooldown that do not exist. The fact now has
 * one implementation, `botStatMaxLevel`, and this reads it.
 *
...
export const BOT_STAT_MAX_LEVEL: Readonly<Record<string, Readonly<Record<string, number>>>>
  = Object.fromEntries(BOT_TABLES.map(bot => [
    ...
    Object.fromEntries(bot.statOrder.map(stat => [
      stat,
      botStatMaxLevel(bot as unknown as BotData, stat),
    ])),
  ]))
...
export const BOT_PLUS_ABILITIES: readonly BotPlusAbility[] = BOT_TABLES
  ...
  .map(bot => {
    ...
    return {
      bot: bot.name,
      ability,
      maxLevel: plus.stats[ability]?.maxLevel ?? 0,
...
/**
 * One node per bot, built from the catalog.
 *
 * Every ultimate weapon had its own node and no bot did: the graph described bot RANGE, bot
 * COOLDOWN and the upgrade ladder, and then had nothing to say about Flame Bot. Anything asking
 * "what is Amplify Bot" got the generic answer for all five, which is exactly the shape of
 * question this graph exists to answer specifically.
 *
 * Generated rather than typed out, because the facts are already in `BOT_UPGRADES_DATA` and a
 * hand-written copy of a stat order is a copy that can disagree with the calculator using it.
 * What is written by hand is the part the catalog cannot hold: what makes each one different, and
 * what people get wrong about it.
 */
const BOT_NOTES: Record<string, { readonly role: string, readonly confusion: string }> = {
  'Flame Bot': {
    role: 'The only bot that upgrades Damage Reduction and Damage, which is why it reads as a defensive bot and a damage bot at once.',
    confusion: 'Its Damage Reduction is a bot stat, not the tower Defense % — they stack rather than replace, and reading one for the other overstates survival.',
  },
  'Thunder Bot': {
    role: 'The only bot with Linger, so its value is time on target rather than a multiplier.',
    confusion: 'Linger is not Duration. Duration is how long the bot is active; Linger is how long its effect remains after it leaves.',
  },
  'Golden Bot': {
    role: 'A coin bot: its Bonus multiplies what enemies pay while it is active, so its worth is Duration times Bonus against its Cooldown.',
    confusion: 'Its Bonus is a coin multiplier and shares a stat name with Amplify Bot and Bot Bot, which multiply something else entirely.',
  },
  'Amplify Bot': {
    role: 'Amplifies the tower rather than the economy, and carries the largest base Bonus of the three Bonus bots.',
    confusion: 'Shares Duration, Cooldown, Bonus and Range with Golden Bot and Bot Bot, so a shared stat order says nothing about what the Bonus multiplies.',
  },
  'Bot Bot': {
    role: 'Multiplies the other bots, so it is worth nothing on its own and scales with how many bots are already strong.',
    confusion: 'Its Bonus is the smallest of the three by base value, which reads as the weakest bot until you notice what it multiplies.',
  },
}

export const PER_BOT_NODES: readonly KnowledgeNode[] = BOT_TABLES.map((bot) => {
  const notes = BOT_NOTES[bot.name]
  const id = `bot.${bot.name.replace(/\s+/g, '')}`
  const maxLevel = bot.costs.length - 1
  ...
  return {
    ...
    summary:
      `${bot.name} upgrades ${bot.statOrder.join(', ')} with medals, to level ${maxLevel}. `
      + notes.role,
    disambiguation: notes.confusion,
    traps: [
      `Its four stats are ${bot.statOrder.join(', ')} — in that order. A save stores bot levels `
      + 'positionally, so reading them against another bot\'s order silently assigns every level to the wrong stat.',
      'Bots are bought and upgraded with MEDALS in the Event Shop. A cost model reading coins or '
      + 'gems is wrong by an entire economy.',
    ],
    implementedBy: ['BOT_UPGRADES_DATA', 'BOT_UPGRADE_STATS_BY_BOT'],
    assertions: [
      { subject: id, predicate: 'statOrder', value: bot.statOrder.join(', '), provenance: CATALOG_BOTS },
      { subject: id, predicate: 'maxUpgradeLevel', value: maxLevel, provenance: CATALOG_BOTS },
      { subject: id, predicate: 'unlockIndex', value: BOTS.findIndex(entry => entry.name === bot.name), provenance: CATALOG_BOTS },
    ],
    sources: [CATALOG_BOTS],
  }
})
...
export const BOT_KNOWLEDGE_EDGES: readonly KnowledgeEdge[] = [
  /*
   * Each bot belongs to the family, so the generic bot rules reach it.
   *
   * `knowledge.test.ts` refuses an orphaned node, and it is right to: a node nothing links to is
   * unreachable by any traversal, so a question about Flame Bot would never arrive at the upgrade
   * ladder or the medal economy that governs it. The five nodes were added without edges and it
   * caught all five.
   */
  ...BOT_TABLES.map(bot => ({
    from: `bot.${bot.name.replace(/\s+/g, '')}`,
    kind: 'memberOf' as const,
    to: 'bot',
    note: `${bot.name} is one of the ${BOT_TABLES.length} bots, and inherits the medal economy and the upgrade ladder.`,
    sources: [CATALOG_BOTS],
  })),
