 ...
 */
/* ── file map ─────────────────────────────────────────────────────────────────
 * exports    CARD_MAX_LEVEL, CARD_COPIES_PER_STAR, CARD_COPIES_ACQUIRED_PER_STAR, CARD_GEM_COST_PER_STAR, CARD_DRAW_RATE, CARD_MILESTONE_UNLOCKS, CARD_KNOWLEDGE_NODES, CARD_KNOWLEDGE_EDGES
 * depends    src/data/cards/data, src/knowledge/substrate/schema
...
/**
 * The per-level table the game itself builds, in `Cards.Initialization`'s `cardBenefit`.
 *
 * Every shipped `levelValues` row is checked against it by
 * `tests/data/game-code-provenance.test.ts`, so this compartment's numbers are no longer
 * "what the wiki said" for the 28 cards the game has a row for.
 */
const GAME_CARD_BENEFIT_TABLE = {
  origin: 'game',
  ref: 'Cards.Initialization cardBenefit',
...
export const CARD_KNOWLEDGE_NODES: readonly KnowledgeNode[] = [
  ...
  },
  {
    ...
    assertions: [
      ...
      // The claim that matters: these are the game's numbers, not the wiki's. Four were not,
      // and each was wrong by an amount small enough to render — Wave Accelerator L5 (48 for
      // 46), Death Ray's whole ladder, Energy Shield L7 (8 for 8.08).
      { subject: 'card.value', predicate: 'checkedAgainstGameTable', value: true, provenance: GAME_CARD_BENEFIT_TABLE, verification: 'verified_here' as const },
      { subject: 'card.value', predicate: 'cardsWithAGameBenefitRow', value: 32, provenance: GAME_CARD_BENEFIT_TABLE, verification: 'verified_here' as const },
      // Super Tower has a card record and no row: its bonus is computed in code by
      // SuperTowerBonusCalc, so its absence is expected rather than a gap in the table.
      { subject: 'card.value', predicate: 'superTowerHasNoBenefitRow', value: true, provenance: GAME_CARD_BENEFIT_TABLE, verification: 'verified_here' as const },
    ],
    sources: [{ ...WIKI_CARDS, section: 'List of Cards' }, CATALOG_CARDS, GAME_CARD_BENEFIT_TABLE],
  },
  {
    id: 'card.slot',
    label: 'Card slot',
