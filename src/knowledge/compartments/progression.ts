 ...
 */
/* ── file map ─────────────────────────────────────────────────────────────────
 * exports    RELIC_STAT_NAMES, MAX_TIER, TIER_UNLOCK_WAVES, TIER_BATTLE_CONDITIONS_FROM, RELIC_SOURCES, RELIC_PUBLISHED_TOTALS_READ_ON, RELIC_PUBLISHED_TOTALS, RELIC_TOTALS_NAME_ALIASES (+28 more)
 * depends    src/data/battle-conditions/names, src/data/generated, src/data/milestones/unlocks, src/data/relics/data, src/data/tiers/data, src/knowledge/substrate/schema
...
} from '../../data/battle-conditions/names'
import { V283_HEAT_BC_INDEX } from '../../data/generated'
import { MILESTONE_SYSTEM_UNLOCK_ROWS } from '../../data/milestones/unlocks'
import {
 ...
 * `MILESTONE_SYSTEM_UNLOCK_ROWS` also removes the second hand-maintained copy of
 * the six gates. `assertions-are-comparable.test.ts` blocks the string form
 * from coming back.
 */
const MILESTONE_UNLOCK_ASSERTIONS = MILESTONE_SYSTEM_UNLOCK_ROWS.flatMap(unlock => [
  {
 ...
 */
/**
 * After v29 Relics.Initialize re-validation of indices 276–304 (token
 * relics-mtbttmal), the six stats that previously sat below the wiki totals now
 * match or exceed them. Kept as an empty list so the pin stays honest — do not
 * re-seed invented shortfalls.
 */
...
const MILESTONE_GATE_EDGES: readonly KnowledgeEdge[] = MILESTONE_SYSTEM_UNLOCK_ROWS
