 ...
 */
/* ── file map ─────────────────────────────────────────────────────────────────
 * exports    BotCommandOption, BotReplyField, BotReply, BotCommandContext, BotCommand, CalculatorCommandOptions, TowerBotOptions
 * depends    —
 * used by    src/bot/calculator-command, src/bot/create-bot, src/bot/index
...
/** How a calculator builder is turned into a bot command. */
...
/** How a Tower bot is configured when it is built. */
export interface TowerBotOptions {
  readonly commands: readonly BotCommand[]
  /**
   * How many recent replies to keep. `0` disables it.
   *
   * Safe because commands are pure functions of their arguments. A command that reads a
   * save, a sheet or a database is not, and must set `cacheable: false` — see
   * `registerCommand`.
   */
  readonly cacheSize?: number
  /** Name of the help command. Set to `null` to leave help to your own code. */
  readonly helpCommandName?: string | null
}
