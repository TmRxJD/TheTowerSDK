/* ── file map ─────────────────────────────────────────────────────────────────
 * exports    LABS_SAVE_UNLOCKED_KEY, LABS_SAVE_RESEARCH_LEVEL_KEY, LABS_SAVE_RESEARCH_PERCENT_KEY, LABS_SAVE_LAB_LEVEL_KEY, LABS_SAVE_LAB_RESEARCH_INT_KEY, LABS_SAVE_LAB_ACTIVE_KEY, LABS_SAVE_LAB_SPEED_UP_SPEED_KEY, LABS_SAVE_LAB_SPEED_UP_REMAINING_KEY (+14 more)
 * depends    src/data/labs/display-overrides, src/data/labs/labs, src/data/labs/research, src/data/player-stats/data, src/internal/persistence/labs-persistence, src/save/access/values (+2 more)
 * used by    src/mechanics/calculators/specs-generated, src/save/effective-paths/from-save, src/save/import/counts, src/save/import/planner, src/save/labs/index
 * ─────────────────────────────────────────────── generated by `npm run map` ──
 */
import { estimateLabLevelRemainingSeconds } from './remaining'
import { findLabResearchByIndex } from '../../data/labs/research'
import {
  isLabsResearchLabName,
  TOOL_LAB_OVERVIEW_CATEGORY_ORDER,
} from '../../data/labs/labs'
import { normalizeLabsLabLevelMap } from '../../internal/persistence/labs-persistence'
import { RESEARCH_CATEGORY_ENUM } from '../../data/player-stats/data'
import {
...
import { coerceSaveNumber, toNumberArray } from '../access/values'
import { readFavoriteLabSlugsFromSaveRoot, saveHasFavoriteLabs } from './favorites'
...
export function readLabsFromSaveRoot(root: Record<string, unknown> | null): LabsSaveExtract | null {
  ...
  if (researchLevels.length > 0 && researchLevels.length !== labResearchImportRowCount()) {
    const expected = labResearchImportRowCount()
    if (researchLevels.length < expected) {
      warnings.push(
        `researchLevel has ${researchLevels.length} entries; catalog has ${expected}. `
          + `Labs ${researchLevels.length}-${expected - 1} are absent from this save, not unresearched.`,
      )
    } else {
      warnings.push(
        `researchLevel has ${researchLevels.length} entries; catalog expects ${expected}.`,
      )
    }
  }
...
export interface LabsSaveImportPayload {
...
export function buildLabsImportPayload(extract: LabsSaveExtract): LabsSaveImportPayload {
  const rawLabLevels: Record<string, number> = {}
  for (const row of extract.researches) {
    if (!row.slug || row.level <= 0) continue
    if (!isLabsResearchLabName(row.slug)) continue
    rawLabLevels[row.slug] = row.level
  }
  const currentLabLevels = normalizeLabsLabLevelMap(rawLabLevels)
  ...
  const activeSlots = Array.from({ length: 5 }, (_, slotIndex) => {
    ...
    if (!researchSlug || !isLabsResearchLabName(researchSlug)) {
      return {
        name: null,
        progressSeconds: 0,
        running: false,
  ...
  for (const slot of extract.activeQueue) {
    ...
    if (!researchSlug || !isLabsResearchLabName(researchSlug) || !slot.speedUp) continue
  ...
  return {
    ...
    favoriteLabs: extract.hasFavoriteLabs
      ? extract.favoriteLabs.filter(slug => isLabsResearchLabName(slug))
      : null,
  }
}
