/* ── file map ─────────────────────────────────────────────────────────────────
 * exports    GUARDIANS_SAVE_UNLOCKED_KEY, GUARDIANS_SAVE_SLOTS_UNLOCKED_KEY, GUARDIANS_SAVE_CHIP_SLOT_KEY, GUARDIANS_SAVE_CHIP_UNLOCKED_KEY, GUARDIANS_SAVE_CHIP_LEVEL_KEY, GUARDIANS_SAVE_SKIN_INDEX_KEY, GUARDIANS_SAVE_SKIN_UNLOCKED_KEY, GUARDIANS_SAVE_DAMAGE_KEY (+13 more)
 * depends    src/data/guardians/guardians, src/save/access/values, src/save/catalogs/guardians, src/save/catalogs/indexes
 * used by    src/mechanics/calculators/specs-generated, src/save/guardians/index, src/save/import/counts, src/save/import/planner, src/save/index
...
} from '../../data/guardians/guardians'
import { coerceSaveNumber, readSaveBoolean, readSaveEnumValue, toNumberArray } from '../access/values'
...
/**
 * Game `ChipType` enum length including unreleased Repair (index 9).
 * Saves pad `guardianChipUnlocked` to this; the import catalog stops at Scout.
 */
export const GUARDIAN_CHIP_NATIVE_ENUM_SLOT_COUNT = GUARDIAN_CHIP_SAVE_SLOT_COUNT + 1
...
function buildChipRow(
  ...
  return {
    ...
    targetKey: findGuardianChipTargetKeyBySlotIndex(slotIndex),
...
export function readGuardiansFromSaveRoot(root: Record<string, unknown> | null): GuardiansSaveExtract | null {
  ...
  // Catalog is Scout-inclusive (9). Live saves often pad to the full ChipType
  // enum (10) for unreleased Repair — that is expected, not a decode fault.
  if (
    chipUnlocked.length > 0
    && chipUnlocked.length !== GUARDIAN_CHIP_SAVE_SLOT_COUNT
    && chipUnlocked.length !== GUARDIAN_CHIP_NATIVE_ENUM_SLOT_COUNT
  ) {
    warnings.push(
...
export interface GuardiansSaveImportPayload {
  levels: Record<string, number[]>
  disabled: Record<string, boolean>
}

export function buildGuardiansImportPayload(
  extract: GuardiansSaveExtract,
): GuardiansSaveImportPayload | null {
  ...
  const chipByKey = new Map(
    extract.chips
      .filter(chip => chip.targetKey)
      .map(chip => [chip.targetKey as string, chip]),
  )

  guardianLabels.forEach((label, index) => {
    const targetKey = guardianKeys[index]
    const chip = targetKey ? chipByKey.get(targetKey) : null
    const baseUnlocked = targetKey === 'bounty'
