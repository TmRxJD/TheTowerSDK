/* ── file map ─────────────────────────────────────────────────────────────────
 * exports    ModuleEquippedSubstat, DecodedModuleSaveSubstat, listModuleSaveEffectIds, inferSubstatRarityFromDisplayedValue, MODULE_SAVE_SUBSTAT_SLOT_COUNT, ModuleSaveSubstatSlotPreview, decodeSingleModuleSaveSubstat, buildModuleSaveSubstatSlotPreviews (+25 more)
 * depends    src/data/modules/modules, src/data/modules/substats, src/save/modules/decode, src/save/modules/registry
 * used by    src/mechanics/calculators/specs-generated, src/save/modules/index, src/save/modules/read
 * ─────────────────────────────────────────────── generated by `npm run map` ──
 */
import type { ModuleCategory } from '../../data/modules/modules'
import { decodeModuleSaveEffect } from './decode'
import {
...
/**
 * Extra core substats present in-game but missing from the chart table.
 *
...
function buildSubstatId(category: ModuleCategory, label: string, used: Set<string>): string {
...
const targetSubstatIdsByCategory = new Map<ModuleCategory, Map<string, string>>()

function targetSubstatIdForLabel(category: ModuleCategory, label: string): string {
  let labelMap = targetSubstatIdsByCategory.get(category)
  if (!labelMap) {
    ...
    for (const definition of MODULE_SUBSTAT_CANONICAL_DATA[canonicalCategory].substats) {
      labelMap.set(definition.label, buildSubstatId(category, definition.label, used))
    }
    for (const label of EXTRA_CORE_SUBSTATS) {
      labelMap.set(label, buildSubstatId('Core', label, used))
    }
    targetSubstatIdsByCategory.set(category, labelMap)
  }
  return labelMap.get(label) ?? buildSubstatId(category, label, new Set(labelMap.values()))
}
...
export function decodeSingleModuleSaveSubstat(
  category: ModuleCategory,
  effectId: number,
): { label: string; targetSubstatId: string; rarity: string } | null {
  if (!effectId) return null

  const decoded = decodeModuleSaveEffect(effectId, category)
  if (!decoded) return null

  const targetSubstatId = targetSubstatIdForLabel(category, decoded.label)
  if (!targetSubstatId) return null

  return { label: decoded.label, targetSubstatId, rarity: decoded.rarity }
}
...
export function decodeModuleSaveSubstats(
  ...
  for (let slotIndex = 0; slotIndex < effects.length; slotIndex += 1) {
    ...
    decoded.push({
      ...
      targetSubstatId: single?.targetSubstatId ?? null,
...
export function buildModuleEquippedSubstatsFromSave(
  ...
  for (let slotIndex = 0; slotIndex < Math.min(effects.length, MODULE_SAVE_SUBSTAT_SLOT_COUNT); slotIndex += 1) {
    ...
    substats.push({
      id: `${decoded.targetSubstatId}-${slotIndex}`,
      type: decoded.targetSubstatId,
      rarity: decoded.rarity,
    })
  }
