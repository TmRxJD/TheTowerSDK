 ...
 * `finishImportPersistence` is the single durable write. So the cheapest
 ...
 */
/* ── file map ─────────────────────────────────────────────────────────────────
 * exports    IdsBlock, IdsLabRow, IdsOutOfRangeLab, IdsLabsExtract, asLevel, readIdsBlocks, resolveIdsLabSaveIndex, idsLabNameAliases (+1 more)
 * depends    src/data/labs/display-overrides, src/data/labs/labs, src/save/ids/grid
...
import { aliasFor, asText, cell, type Grid } from './grid'
import { normalizeToolLabLookupKey } from '../../data/labs/labs'
...
export interface IdsLabRow {
   ...
   * level with nothing. Null means "leave the stored level alone".
   */
  level: number | null
  /**
   * The level the player is AIMING for, from the column between level and cap.
   *
   * A separate quantity from both: progress stores a per-lab target range,
 ...
 * guess at it overwrites a real stored level with a wrong one. Negative is not
 ...
 *
 * NOT via `LAB_RESEARCH_LAB_BY_LEGACY_LEVEL_KEY`, which looks like it was written for
...
export function readIdsLabs(grid: Grid): IdsLabsExtract {
  ...
  for (let row = 1; row < grid.length; row += 1) {
    ...
    // v5.10.02 interleaves a `Presets` card-preset-count row into the labs column;
    // it is metadata, not a lab, so it is neither collected nor reported unresolved.
