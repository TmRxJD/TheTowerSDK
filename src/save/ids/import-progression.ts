 ...
 */
/* ── file map ─────────────────────────────────────────────────────────────────
 * exports    GUARDIAN_EQUIP_SLOT_COUNT, GUARDIAN_CHIP_SLOT_EMPTY, GUARDIAN_UPGRADES_PER_CHIP, IdsGuardianRow, resolveIdsGuardianChipIndex, readIdsGuardians, BOT_UPGRADES_PER_BOT, IdsBotRow (+12 more)
 * depends    src/data/assets/data, src/data/player-stats/data, src/data/workshop/enhancement-progress-definitions, src/data/workshop/progress-definitions, src/save/cards/read, src/save/catalogs/guardians (+8 more)
 * used by    src/mechanics/calculators/specs-generated, src/save/ids/import-collections, src/save/ids/index
...
export interface IdsBotRow {
  ...
  isPlus: boolean
  /** Position in DISPLAY order, which is the order the sheet lists. */
 ...
 * are positional, in display order, and are converted to the save's order by
 ...
 *
 * The join uses `HARMONY_VAULT_SLOT_BINDINGS`, the repo's own mapping,
 * so what this writes is what a vault reader reads back.
 *
...
export function readIdsVaultUnlocks(grid: Grid): IdsDomainExtract<IdsVaultUnlockRow> {
  ...
  for (const binding of HARMONY_VAULT_SLOT_BINDINGS) {
 ...
 *
 * The lifetime record holds twenty-two fields. The sheet carries four of
