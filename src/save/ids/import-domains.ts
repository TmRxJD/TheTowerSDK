 ...
 * composables and `finishImportPersistence` do their own jobs unchanged.
 ...
 * hold what a progress view needs (Cards levels, Workshop levels); others hold EP's
 ...
 * array its target stores, so neither is mapped. `idsDomainCoverage()` reports
 * that rather than leaving it to be discovered as an empty target.
 */
/* ── file map ─────────────────────────────────────────────────────────────────
 * exports    readIdsBotUnlocked, IdsCardRow, resolveIdsCardSaveIndex, readIdsCards, IdsWorkshopRow, IDS_WORKSHOP_STATS_NOT_MODELLED, resolveIdsWorkshopSlot, readIdsWorkshop (+8 more)
 * depends    src/data/assets/data, src/data/player-stats/data, src/data/workshop/enhancement-progress-definitions, src/data/workshop/progress-definitions, src/save/cards/read, src/save/catalogs/guardians (+7 more)
 * used by    src/mechanics/calculators/specs-generated, src/save/ids/import-collections, src/save/ids/import-progression, src/save/ids/index
 * ─────────────────────────────────────────────── generated by `npm run map` ──
 */
import { aliasFor, asNumber, asText, cell, type Grid, normalizeName } from './grid'
...
import { getWorkshopEnhancementDefinitions } from '../../data/workshop/enhancement-progress-definitions'
import { getWorkshopStatDefinitions } from '../../data/workshop/progress-definitions'
import { findCardCatalogFromSaveIndex } from '../cards/read'
import { listGuardianChipSlotCatalogRows } from '../catalogs/guardians'
import { HARMONY_VAULT_SLOT_BINDINGS } from '../catalogs/vault'
...
export function readIdsBotUnlocked(grid: Grid, block: IdsBlock, nameRow: number): boolean | null {
  ...
  // Current masters (v5.10.02+): per-preset ownership booleans sit directly under
  // each `Active` label, one per preset slot. A bot the player owns on preset 2
  // with an empty preset 1 reads `false | true`, so it is unlocked if ANY preset
  // toggle is true; all-false means genuinely locked.
  //
  // Older masters (v5.09) put a level STRING under the early `Active` labels and
  // only a stray trailing boolean far to the right, which is not an ownership
  // toggle. Require the FIRST `Active` to carry a boolean before trusting the
  // row — otherwise that trailing artifact reads a bot the player owns as locked.
  let firstActive = -1
  for (let col = block.fromColumn; col <= block.toColumn; col += 1) {
    if (asText(cell(grid, nameRow, col)) === 'Active') { firstActive = col; break }
  }
  if (firstActive < 0 || typeof cell(grid, nameRow + 1, firstActive) !== 'boolean') return null

  for (let col = firstActive; col <= block.toColumn; col += 1) {
    if (asText(cell(grid, nameRow, col)) !== 'Active') continue
    if (cell(grid, nameRow + 1, col) === true) return true
  }
  return false
}
...
export interface IdsWorkshopRow {
   ...
   * writing it into `upgradeWorkshopLevel` would have claimed a player
