/* ── file map ─────────────────────────────────────────────────────────────────
 * exports    VAULT_SAVE_KEYS_KEY, VAULT_SAVE_TOTAL_KEYS_EARNED_KEY, VAULT_SAVE_POWER_LEVEL_KEY, VAULT_SAVE_POWER_MAX_LEVEL_KEY, VAULT_SAVE_POWER_UNLOCKED_KEY, VAULT_SAVE_HARMONY_UNLOCKED_KEY, VAULT_SAVE_TIER2_UNLOCK_KEY, VAULT_SAVE_TIER3_UNLOCK_KEY (+13 more)
 * depends    src/data/player-stats/data, src/data/vault/tree-traversal, src/save/access/values, src/save/catalogs/vault-overrides, src/save/vault/v29-progress
 * used by    src/mechanics/calculators/specs-generated, src/save/import/counts, src/save/import/planner, src/save/index, src/save/shared-tool-inputs/from-save-extended, src/save/vault/index
...
import { coerceSaveNumber, readSaveBoolean, toNumberArray } from '../access/values'
import {
  readVaultV29ProgressFromSaveRoot,
  vaultV29RowKey,
  type VaultV29ProgressExtract,
} from './v29-progress'

export { vaultV29RowKey }
...
/** Display uses levels 1–3; save `powerNodesLevel` is 0-based (0 = tier 1). */
...
export interface VaultSaveExtract {
  ...
  /**
   * v29 `PlayerData.vault` progress mapped to import keys (`v29:…`).
   * Null when the save predates VaultPlayerProgress.
   */
  v29: VaultV29ProgressExtract | null
  warnings: string[]
}
...
export function readVaultFromSaveRoot(root: Record<string, unknown> | null): VaultSaveExtract | null {
  ...
  const v29 = readVaultV29ProgressFromSaveRoot(root)
  if (v29?.warnings.length) warnings.push(...v29.warnings)

  if (powerLevels.length === 0 && harmonyUnlocked.length === 0 && !v29) {
...
export interface VaultSaveImportPayload {
  levels: Record<string, number>
  spentKeys: number
}

export function buildVaultImportPayload(extract: VaultSaveExtract): VaultSaveImportPayload | null {
  const levels: Record<string, number> = {}
  let hasProgress = false

  // Prefer v29 VaultPlayerProgress — that is what consumer UI keys on.
  if (extract.v29 && Object.keys(extract.v29.levels).length > 0) {
    for (const [key, level] of Object.entries(extract.v29.levels)) {
      if (level <= 0) continue
      levels[key] = level
      hasProgress = true
    }
  } else {
    // Pre-v29 saves: legacy parallel arrays keyed by old node ids.
