/* ── file map ─────────────────────────────────────────────────────────────────
 * exports    RunDataRecordLike, CanonicalRunData, canonicalizeRunData, serializeRunForCloudAttributes, getFirstMeaningfulRunDataValue, applyRunDataAliasGroups, normalizeGuardianSummonedEnemies, dedupeEquivalentRunDataKeys (+1 more)
 * depends    src/formatting, src/save/runs/field-vocabulary
 * used by    src/mechanics/calculators/run, src/mechanics/calculators/specs-generated, src/save/runs/index
 * ─────────────────────────────────────────────── generated by `npm run map` ──
 */
import { z } from 'zod'
import { standardizeNotation } from '../../formatting'
import { type RunDataAliasGroup, TRACK_RUN_OUTPUT_ALIAS_GROUPS } from './field-vocabulary'
...
const CANONICAL_RUN_STRING_FIELDS = [
  'tier',
  'tierDisplay',
  'wave',
  'roundDuration',
...
const CANONICAL_RUN_METRIC_FIELDS = new Set<string>([
  'totalCoins',
  'totalCells',
  'totalDice',
  'coinsPerHour',
...
const CANONICAL_RUN_MAX_20_FIELDS = new Set<string>([
  ...CANONICAL_RUN_METRIC_FIELDS,
  'gameTime',
])

const canonicalRunOptionalFieldShape: Record<string, z.ZodTypeAny> = Object.fromEntries(
  CANONICAL_RUN_STRING_FIELDS
    .filter(key => ![
...
const canonicalRunSchema = z.object({
  ...
  ...canonicalRunOptionalFieldShape,
}).strip()

export type CanonicalRunData = z.infer<typeof canonicalRunSchema>
...
function normalizeCanonicalFieldValue(key: string, value: unknown): unknown {
  ...
  if (CANONICAL_RUN_METRIC_FIELDS.has(key)) {
...
function buildCanonicalFieldMap(data: RunDataRecordLike): Record<string, unknown> {
  ...
  for (const key of CANONICAL_RUN_STRING_FIELDS) {
...
export function canonicalizeRunData(data: RunDataRecordLike): CanonicalRunData {
  return canonicalRunSchema.parse(buildCanonicalFieldMap(data))
}

export function serializeRunForCloudAttributes(data: RunDataRecordLike): Record<string, unknown> {
  const canonical = canonicalizeRunData(data)
  ...
  for (const [key, value] of Object.entries(canonical)) {
    ...
    const text = toTrimmedString(value)
    if (!text) continue
    if (CANONICAL_RUN_MAX_20_FIELDS.has(key) && text.length > 20) continue
    serialized[key] = text
  }
