/* ── file map ─────────────────────────────────────────────────────────────────
 * exports    readTradeOffPerksFromSaveRoot, readLabRelicPctFromSaveRoot, readGemDiscountMultiplier, readLabSpeedUpFromSaveRoot, readTowerRangeMetersFromSaveRoot, readBotBotBonusMultiplier, readDeathWaveBaseWavesFromSaveRoot, readUptimeModuleRaritiesFromSaveRoot (+2 more)
 * depends    src/data/bots/data, src/data/labs/labs, src/data/modules/levels, src/formatting/numbers/compute, src/internal/shared-tool-inputs/from-research, src/internal/shared-tool-inputs/shared-tool-inputs (+10 more)
 * used by    src/mechanics/calculators/specs-generated, src/save/shared-tool-inputs/index
 * ─────────────────────────────────────────────── generated by `npm run map` ──
 */
import { roundToDisplayPrecision } from '../../formatting/numbers/compute'
import { BOT_UPGRADES_DATA, botStatValueAt } from '../../data/bots/data'
...
import { computeLabValueAtLevel, getSharedToolLabs } from '../../data/labs/labs'
...
} from '../../internal/shared-tool-inputs/shared-tool-inputs'
import {
  mergeSharedUptimeInputs,
  type SharedUptimeInputs,
  type SharedUptimeRarityPick,
  UPTIME_UW_FIELD_MAP,
} from '../../internal/uptime/shared-inputs'
import { getOutOfRoundMaxDistance } from '../../mechanics/combat/tower-range'
import { RANGE_WORKSHOP_UPGRADE_INDEX } from '../../mechanics/primitives/constants'
 ...
 *
 * Save is `[secondary, range, cooldown, primary]`; display order is
...
export function readBotBotBonusMultiplier(root: Record<string, unknown>): number {
  ...
  const targetLevels = remapBotLevelsFromGameSaveOrder(saveSlice)
  const bonusLevel = targetLevels[2] ?? 0
  if (bonusLevel <= 0) return 0

  const botBot = BOT_UPGRADES_DATA.find(bot => bot.label === 'Bot Bot')
  const stat = botBot?.stats.Bonus
  if (!stat) return 0
  return botStatValueAt(stat, bonusLevel)
}
