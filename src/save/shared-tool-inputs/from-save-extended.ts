/* ── file map ─────────────────────────────────────────────────────────────────
 * exports    DISSONANCE_BOOST_SAVE_FIELD_BY_TYPE, DissonanceCycleLayout, readWorkshopLevelByTargetKey, readWorkshopEnhancementLevelByTargetKey, mapModuleSaveRarityEnum, readResearchLabLevelsFromSaveRoot, readWorkshopStatLevelsFromSaveRoot, readVaultLevelsFromSaveRoot (+25 more)
 * depends    src/data/cards/data, src/data/labs/display-overrides, src/data/labs/research, src/data/modules/levels, src/data/player-stats/data, src/data/tiers/campaign (+18 more)
 * used by    src/mechanics/calculators/specs-generated, src/save/dissonance/read, src/save/shared-tool-inputs/from-save, src/save/shared-tool-inputs/index
 * ─────────────────────────────────────────────── generated by `npm run map` ──
 */
import { LAB_RESEARCH_BY_INDEX } from '../../data/labs/research'
import { CARD_TEMPLATES } from '../../data/cards/data'
...
import { CARD_IMPORT_CATALOG } from '../../data/player-stats/data'
import {
...
import { decodeModuleSaveEffect, type ModuleSaveSlotCategory } from '../modules/decode'
import {
...
import { ELS_ATTACK_WORKSHOP_KEY, ELS_HEALTH_WORKSHOP_KEY } from '../../mechanics/modules/els-upgrade-path'
...
import { CARDS_SAVE_UNLOCKED_KEY } from '../cards/read'
import { readModulesFromSaveRoot } from '../modules/read'
...
export function readWorkshopLevelByTargetKey(
  root: Record<string, unknown>,
  targetKey: string,
): number | null {
  const row = WORKSHOP_IMPORT_CATALOG.find(entry => entry.targetKey === targetKey)
...
export function readWorkshopEnhancementLevelByTargetKey(
  root: Record<string, unknown>,
  targetKey: string,
): number | null {
  const row = WORKSHOP_ENHANCEMENT_IMPORT_CATALOG.find(entry => entry.targetKey === targetKey)
  if (!row?.saveField || row.categoryIndex == null) return null
  const levels = readWorkshopArray(root, row.saveField)
  const level = levels[row.categoryIndex]
  return Number.isFinite(level) ? Math.max(0, Math.floor(level)) : null
...
export function readWorkshopStatLevelsFromSaveRoot(
  ...
  for (const row of WORKSHOP_ENHANCEMENT_IMPORT_CATALOG) {
    ...
    const level = saveLevels[row.categoryIndex]
    if (!Number.isFinite(level) || level <= 0) continue
    enhancementLevels[row.targetKey] = Math.floor(level)
  }
...
export function readVaultLevelsFromSaveRoot(root: Record<string, unknown>): SharedVaultLevels {
  ...
  if (!extract) {
    return { levels: {}, spentKeys: 0 }
  }
  const payload = buildVaultImportPayload(extract)
  return {
    levels: payload?.levels ?? {},
    spentKeys: payload?.spentKeys ?? 0,
  }
}
...
export function readElsPlannerInputsFromSaveRoot(
  ...
  const currentTier = coerceSaveNumber(root.currentTier)
  const highestWaveThisTier = toNumberArray(root.highestWaveThisTier)
  const attackSkip = readWorkshopLevelByTargetKey(root, ELS_ATTACK_WORKSHOP_KEY)
  const healthSkip = readWorkshopLevelByTargetKey(root, ELS_HEALTH_WORKSHOP_KEY)
...
export function readModuleProgressFromSaveRoot(
  ...
  // Prefer the canonical modules reader so v29 slotLevels migration is applied once.
  const extract = readModulesFromSaveRoot(root)
  if (extract) {
    for (const item of extract.equipped) {
      const moduleType = MODULE_CATEGORY_TO_TYPE[item.category as ModuleSaveSlotCategory]
      if (!moduleType) continue
      if (item.role === 'primary') {
        if (item.level != null && item.level > 0) currentLevel[moduleType] = Math.floor(item.level)
        if (item.rarityLabel) lastRarityByType[moduleType] = item.rarityLabel
      } else {
        if (item.level != null && item.level > 0) assistCurrentLevel[moduleType] = Math.floor(item.level)
        if (item.rarityLabel) lastAssistRarityByType[moduleType] = item.rarityLabel
      }
    }
  }
...
export function readThornsCalculatorSettings(
  ...
  const thornLevel = readWorkshopLevelByTargetKey(root, 'Thorn Damage')
    ?? readWorkshopLevelByTargetKey(root, 'Thorns')
  const currentTier = coerceSaveNumber(root.currentTier)
...
/**
 * Bot synchronicity (slot ownership, targets, assignments) is caller-local today.
