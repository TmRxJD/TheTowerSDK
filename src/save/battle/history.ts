 ...
 * depends    src/save/access/values, src/save/battle/history-normalize
 * used by    src/mechanics/calculators/specs-generated, src/node/decode-save, src/save/access/path, src/save/battle/index, src/save/battle/reports, src/save/import/counts (+2 more)
 * ─────────────────────────────────────────────── generated by `npm run map` ──
 */
import { readSaveListItems } from '../access/values'
...
export function listImportableBattleRuns(parsedRoot: unknown): Record<string, unknown>[] {
  ...
  return items
    .filter(looksLikeBattleRun)
    .map(entry => normalizeBattleHistorySaveEntry(entry as Record<string, unknown>))
}
