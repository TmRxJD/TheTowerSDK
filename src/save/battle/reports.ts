/* ── file map ─────────────────────────────────────────────────────────────────
 * exports    readBattleDateFromSave, normalizeBattleRunDateTimeForDedup, buildBattleRunDedupKeysFromStoredRun, buildBattleRunDeduplicationKey, isPopulatedBattleRunDedupKey, normalizeBattleRunDateForDedup, normalizeBattleRunDurationForDedup, normalizeBattleRunCompactForDedup (+7 more)
 * depends    src/formatting, src/formatting/tools/compute, src/save/battle/duration, src/save/battle/history, src/save/battle/history-normalize, src/save/battle/report-fields (+2 more)
 * used by    src/mechanics/calculators/specs-generated, src/save/battle/index, src/save/import/discovery, src/save/import/planner
 * ─────────────────────────────────────────────── generated by `npm run map` ──
 */
import { formatBattleDurationFromSaveSeconds, readDurationSecondsFromSave } from './duration'
import { buildBattleReportStatFields } from './report-fields'
import { normalizeBattleHistorySaveEntry } from './history-normalize'
import { parseDurationToHours, parseSaveDateTimeToMs } from '../../formatting'
import { listImportableBattleRuns } from './history'
import { getKilledByFromSave } from '../killed-by/read'
import { normalizeRunDateText, normalizeRunTimeText } from '../runs/fields'
...
function collectBattleRunTimestampMsCandidates(run: Record<string, unknown>): number[] {
  ...
  const runDate = normalizeRunDateText(run.runDate ?? run.date)
  const runTime = normalizeRunTimeText(run.runTime ?? run.time)
...
export function planBattleReportImport(
  ...
  for (const entry of entries) {
    ...
    existingKeys.add(key)
    importable.push(buildRunData(entry))
  }
