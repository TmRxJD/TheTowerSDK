/**
 * Numbers formatted for reading next to each other.
 *
 * Separate from `numbers/` because these choose a PRESENTATION rather than answering what a
 * number is: a coin total, a shard count, a multiplier. The distinction matters because the
 * presentation is a preference and the value is not — a caller may want `1.5M` or `1,500,000`
 * for the same number, and neither is more correct.
 *
 * Each helper is a fixed pairing of grouping, fraction digits and fallback, so that two views
 * showing the same quantity cannot disagree about how it looks.
 */
/* ── file map ─────────────────────────────────────────────────────────────────
 * exports    GroupedToolNumberOptions, formatCompact, formatLargeNumber, formatGroupedToolNumber, formatOptionalGroupedToolNumber, formatShard, formatCoin, formatMultiplier (+6 more)
 * depends    src/formatting/numbers
 * used by    src/formatting/tools/index, src/mechanics/calculators/specs-generated, src/mechanics/resource-drops/enemy-drop-roi, src/save/battle/report-fields, src/save/battle/reports, src/save/shared-tool-inputs/from-save-extended
