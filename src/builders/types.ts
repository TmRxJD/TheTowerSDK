 ...
 */
/* ── file map ─────────────────────────────────────────────────────────────────
 * exports    CalculatorField, CalculatorResultBase, CalculatorBuilder, MAX_INPUT_MAGNITUDE, MAX_MODEL_WAVE, clampLevel, clampMagnitude, costIndexForLevel (+4 more)
 * depends    —
 ...
 *
 * `normalize(input = {})` defaults only for `undefined`. A JSON API, a form library and this
 * package's own WebAssembly entry all produce `null` routinely, and every calculator threw on it —
 * while the documentation promised `normalize` "takes anything and returns a complete, valid
 * record. Nothing throws."
 *
 * It also copies own enumerable properties one at a time, swallowing any whose getter throws. A
 * value that throws when COERCED was already covered; a property that throws when READ is a
 * different thing, and three calculators died on it. Reading someone else's object means running
 * their code.
 *
 * Own keys only, so `__proto__` and `constructor` arrive as ordinary absent properties rather than
