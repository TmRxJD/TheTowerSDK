 ...
 */
/* ── file map ─────────────────────────────────────────────────────────────────
 * exports    guardianCalculator
 * depends    src/builders/types, src/data
 * used by    src/builders/index
 * ─────────────────────────────────────────────── generated by `npm run map` ──
 */
import { buildGuardianDefinitions, getGuardianStatCostAt, type GuardianDefinition } from '../data'
import { type CalculatorBuilder, type CalculatorResultBase, clampNumber ,
  safeRecord} from './types'
import type { GuardianInput, GuardianLevel, GuardianResult } from './types'
...
export const guardianCalculator: CalculatorBuilder<GuardianInput, GuardianResult> = {
  ...
  compute(rawInput = {}) {
    ...
    if (typeof asked.stat === 'string' && asked.stat && asked.stat !== input.stat) {
      notes.push(`${guardian.label} has no stat named "${asked.stat}"; used ${input.stat} instead.`)
    }
