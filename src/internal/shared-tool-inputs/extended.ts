/* ── file map ─────────────────────────────────────────────────────────────────
 * exports    sharedWorkshopStatLevelsSchema, SharedWorkshopStatLevels, defaultSharedWorkshopStatLevels, sharedEnemyStatsCoreSchema, SharedEnemyStatsCore, defaultSharedEnemyStatsCore, sharedElsPlannerInputsSchema, SharedElsPlannerInputs (+47 more)
 * depends    src/data/tournaments/data, src/internal/cards-progress-inputs, src/internal/local-state/bot-medal-splitter, src/internal/local-state/calculator-local-state-schemas, src/internal/local-state/dissonance-calcs, src/internal/local-state/enemy-drops-calcs (+6 more)
 * used by    src/internal/shared-tool-inputs/field-sync, src/internal/shared-tool-inputs/hub-merge, src/internal/shared-tool-inputs/shared-tool-inputs, src/internal/shared-tool-inputs/storage, src/mechanics/calculators/specs-generated, src/mechanics/enemies/drops-context (+5 more)
...
} from '../local-state/enemy-stats-calcs'
import {
...
} from '../local-state/enemy-drops-calcs'
import {
...
} from '../local-state/calculator-local-state-schemas'
import {
...
import { normalizeTierSelection, type TierSelectionInput } from '../../data/tournaments/data'
import { type UwProgressLevels, uwProgressLevelsSchema } from '../uptime/shared-inputs'
...
/** A 0..1 preference weight, falling back rather than clamping nonsense to an edge. */
...
export const sharedBotMedalSplitterPlannerSchema = z.object({
  ...
  presets: z.array(z.object({
    ...
    botAxisWeights: z.record(z.string(), z.number()).catch({}),
    // `.catch` because this replaced `useBotBotPlus`, and the surrounding schema is parsed
    // with a THROWING `.parse`. A required boolean here would reject every preset saved before
    // the rename — taking the whole payload with it, not just this field.
    includeBotPlus: z.boolean().catch(true),
    ...
    botValueWeights: z.record(z.string(), z.number()).catch({}),
    botSyncPartners: z.record(z.string(), z.array(z.string())).catch({}),
...
export type { SharedEnemyDropsInputs } from '../local-state/enemy-drops-calcs'
export {
  defaultSharedEnemyDropsInputs,
  normalizeSharedEnemyDropsInputs,
} from '../local-state/enemy-drops-calcs'
...
export function readSharedBotMedalSplitterPlanner(input: {
  ...
  return {
    ...
    presets: input.presets.map(preset => ({
      ...
      botAxisWeights: { ...preset.botAxisWeights },
      includeBotPlus: preset.includeBotPlus,
      ...
      botValueWeights: { ...preset.botValueWeights },
      botSyncPartners: Object.fromEntries(Object.entries(preset.botSyncPartners).map(([label, partners]) => [label, [...partners]])),
    ...
    normalized.botMedalSplitterPlanner = {
      ...
      presets: medalPlanner.presets.slice(0, BOT_MEDAL_SPLITTER_PRESET_COUNT).map((preset, index) => {
        ...
        return {
          ...
          botAxisWeights: Object.fromEntries(Object.entries({
            ...fallback.botAxisWeights,
            ...(sourcePreset.botAxisWeights ?? {}),
          }).map(([label, weight]) => [label, clampRatio(weight, fallback.botAxisWeights[label] ?? 0)])),
          includeBotPlus: typeof sourcePreset.includeBotPlus === 'boolean'
            ? sourcePreset.includeBotPlus
            : sourcePreset.useBotBotPlus !== false,
          botBotEnabled: sourcePreset.botBotEnabled === true,
          botBotSynced: sourcePreset.botBotSynced === true,
          botValueWeights: { ...(sourcePreset.botValueWeights ?? {}) },
          botSyncPartners: Object.fromEntries(Object.entries(sourcePreset.botSyncPartners ?? {}).map(([label, partners]) => [label, [...(partners ?? [])]])),
          targetConfigs: {},
