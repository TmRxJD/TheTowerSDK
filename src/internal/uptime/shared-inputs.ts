/* ── file map ─────────────────────────────────────────────────────────────────
 * exports    uptimeRarityPickSchema, SharedUptimeRarityPick, uptimeCompressorSchema, uptimeMvnModeSchema, UptimeBotFieldMap, UPTIME_BOT_FIELD_MAP, UptimeGuardianFieldMap, UPTIME_GUARDIAN_FIELD_MAP (+28 more)
 * depends    src/data/bots/data, src/data/guardians/guardians, src/data/guardians/upgrades, src/data/ultimate-weapons/ultimate-weapons, src/internal/uptime/persistence, src/internal/uptime/state-builder (+2 more)
 * used by    src/internal/shared-tool-inputs/extended, src/internal/shared-tool-inputs/from-research, src/internal/shared-tool-inputs/hub-merge, src/internal/shared-tool-inputs/shared-tool-inputs, src/internal/shared-tool-inputs/storage, src/mechanics/calculators/specs-generated (+4 more)
 * ─────────────────────────────────────────────── generated by `npm run map` ──
 */
import { z } from 'zod'
import { BOT_UPGRADES_DATA, type BotData } from '../../data/bots/data'
import { buildGuardianDefinitions } from '../../data/guardians/guardians'
import { guardianUpgrades } from '../../data/guardians/upgrades'
import { normalizeUptimePersistedPayload } from './persistence'
import { buildUptimeCoreStateFromRecord } from './state-builder'
import type { UptimeCoreState } from '../../mechanics/uptime/compute'
import { findUwStatByName, findUwWeaponByName, type UwWeaponValue } from '../../data/ultimate-weapons/ultimate-weapons'
...
import { UPTIME_UW_FIELD_MAP, type UptimeUwFieldMap } from './uw-field-map'
...
export function applySharedUptimeInputs(
  ...
  for (const [key, value] of Object.entries(inputs)) {
    ...
    // Hub payloads normalize missing linked fields to null — never clobber persisted levels.
    if (value === null && isUptimeLinkedLevelField(key) && data[key] != null) continue
    data[key] = value
  }
}
...
export function syncUptimeBotsFromProgress(
...
export function syncProgressBotsFromUptime(
...
export function syncUptimeGuardiansFromProgress(
...
export function syncProgressGuardiansFromUptime(
  uptime: SharedUptimeInputs,
  guardianLevels: Record<string, number[]>,
...
export function isUptimeLinkedLevelField(fieldName: string): boolean {
...
export function syncUptimeUwFromProgress(
  progress: Record<string, { starts?: Record<number, number> }>,
  weapons: UwWeaponValue[],
  existing: SharedUptimeInputs = {},
): SharedUptimeInputs {
...
export function syncProgressUwFromUptime(
  uptime: SharedUptimeInputs,
...
/** Incoming bot levels/labs overwrite linked uptime bot fields (no Math.max — supports lowering values). */
export function overlaySharedUptimeInputsFromProgress(
  ...
  return {
    ...
    ...syncUptimeBotsFromProgress(botLevels, botLabLevels),
  }
}
...
export function buildSharedUptimeInputsFromSources(input: {
  ...
  if (input.botLevels) {
    merged = overlaySharedUptimeInputsFromProgress(
      merged,
      input.botLevels,
      input.botLabLevels ?? {},
    )
  ...
  if (input.guardianLevels) {
    merged = mergeSharedUptimeInputs(merged, syncUptimeGuardiansFromProgress(input.guardianLevels))
  }
  if (input.uwProgress && input.uwWeapons?.length) {
    merged = mergeSharedUptimeInputs(merged, syncUptimeUwFromProgress(input.uwProgress, input.uwWeapons))
  }
  return merged
}
...
export function collectUwProgressLevelsFromUptime(
  ...
  const progress = syncProgressUwFromUptime(uptime, {}, weapons)
