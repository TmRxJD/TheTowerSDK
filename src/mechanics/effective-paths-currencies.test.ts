...
} from './effective-paths-currencies'
...
import { EFFECTIVE_REGEN_UPGRADES } from './effective-paths-regen-plan'
...
describe('the candidate counts match the ported lists', () => {
  ...
  it('matches eDamage on every path', () => {
    expect(countFor('edamage', 'research-time')).toBe(EFFECTIVE_DAMAGE_CANDIDATES.lab.length)
    expect(countFor('edamage', 'power-stones')).toBe(EFFECTIVE_DAMAGE_CANDIDATES.stone.length)
    expect(countFor('edamage', 'keys')).toBe(EFFECTIVE_DAMAGE_CANDIDATES.keys.length)
  })
