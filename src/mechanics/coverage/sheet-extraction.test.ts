...
import inventory from './data/sheet-extraction.v1.json'
...
const MECHANICS = path.resolve(HERE, '..')
...
describe('the sheet extraction denominator', () => {
  ...
  it('still models every function it claims to model', () => {
     ...
     */
    expect(inventory.counts.modelledFunctions).toBe(131)
  })
  ...
  it('splits the unmodelled set instead of quoting one misleading number', () => {
    ...
    // `modelledFunctions`.
    expect(inventory.counts.unmodelledComputation).toBe(10)
