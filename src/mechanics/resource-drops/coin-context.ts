/* ── file map ─────────────────────────────────────────────────────────────────
 * exports    ResourceDropsCoinLinkedSources, assembleResourceDropsCoinSimulationInput, RESOURCE_DROPS_COIN_LAB_SLUGS, coinTradeOffPerkMult
 * depends    src/data, src/internal/cards-progress-inputs, src/internal/local-state/bot-medal-splitter, src/internal/shared-tool-inputs/extended, src/internal/shared-tool-inputs/shared-tool-inputs, src/internal/uptime/shared-inputs (+7 more)
...
import { computeResearchLabLevel, findLabResearchBySlug } from '../../data'
import type {
...
import { BOT_UPGRADES_DATA, formatBotStatValue } from '../../data'
import { DEFAULT_BOT_MEDAL_SPLITTER_TOWER_RANGE } from '../../internal/local-state/bot-medal-splitter'
import { iapTogglesFromLegacyMults } from './coin-iap'
import { computeModuleCoinsKillBonusFromHub } from './coin-module-cpk'
import {
...
} from './coin-generator-modules'
import { introSprintZeroCoinWaveCap } from './coin-intro-sprint'
import {
...
function readGoldenBotRangeValue(gbRangeLevel: number): string {
  ...
  if (!stat) return '20M'
  return formatBotStatValue(stat, gbRangeLevel)
}
