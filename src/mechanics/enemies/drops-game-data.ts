 ...
 */
/* ── file map ─────────────────────────────────────────────────────────────────
 * exports    MODULE_DROP_LAB_MULT, COMMON_MODULE_DROP_BASE, RARE_MODULE_DROP_BASE, BOSS_REROLL_SHARDS_PROC, REROLL_SHARDS_BY_TIER, ModuleShatterRarity, ResolvedEnemyDropsLabs, getCommonModuleDropChance (+15 more)
 * depends    src/data, src/data/enemies/data, src/mechanics/primitives/math
 * used by    src/mechanics/calculators/specs-generated, src/mechanics/enemies/drops-simulation, src/mechanics/enemies/index, src/mechanics/resource-drops/accumulate-wave-econ, src/mechanics/resource-drops/coin-fetch, src/mechanics/resource-drops/enemy-drop-roi (+1 more)
...
import { FLEET_REWARD_ROWS } from '../../data/enemies/data'
...
/** Fleet enemy drop: 80% reroll shard table, 20% module shard branch (oracle). */
export const FLEET_REROLL_SHARD_PROC = 0.8 as const
export const FLEET_MODULE_SHARD_PROC = 0.2 as const
/** Each module type (Cannon/Armor/Generator/Core) rolls independently at 5%. */
export const FLEET_MODULE_SHARD_PROC_PER_TYPE = 0.05 as const
export const FLEET_MODULE_SHARD_TYPE_COUNT = 4 as const

export function expectedFleetModuleShardsEv(
  fleetCount: number,
  moduleShardsPerType: number,
  wsMult: number,
  scope: 'oneType' | 'allTypes' = 'allTypes',
): number {
  const proc = scope === 'oneType' ? FLEET_MODULE_SHARD_PROC_PER_TYPE : FLEET_MODULE_SHARD_PROC
  return fleetCount * proc * moduleShardsPerType * wsMult
}

export function getFleetRewardRow(tier: number): { tier: number; rerollShards: number; moduleShardsPerType: number } | undefined {
  const t = clamp(Math.floor(tier), 1, MAX_CAMPAIGN_TIER)
  return FLEET_REWARD_ROWS.find(row => row.tier === t)
}
