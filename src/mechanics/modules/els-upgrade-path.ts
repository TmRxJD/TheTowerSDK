 ...
 */
/* ── file map ─────────────────────────────────────────────────────────────────
 * exports    ELS_ATTACK_WORKSHOP_KEY, ELS_HEALTH_WORKSHOP_KEY, ELS_ENHANCEMENT_KEY, ELS_ENHANCEMENT_ALIASES, computeElsEnhancementLevel, ElsWorkshopLead, ElsBudgetAllocationRow, formatElsSkipPct (+32 more)
 * depends    src/data/workshop/costs, src/data/workshop/progress-definitions, src/data/workshop/workshop, src/mechanics/enemies/level-skip, src/mechanics/modules/els-module-cluster, src/mechanics/planning/roi-scaling
 * used by    src/internal/shared-tool-inputs/field-sync, src/mechanics/calculators/specs-generated, src/mechanics/modules/els-calculator-options, src/mechanics/modules/index, src/save/shared-tool-inputs/from-save-extended
...
import { computeRoiReferenceCost, formatAdaptiveRoiPct } from '../planning/roi-scaling'
...
/** Save import / legacy workshop target keys for utility ELS+ enhancement. */
export const ELS_ENHANCEMENT_ALIASES = [
  'Enemy Level Skip +',
  'Enemy Level Skips +',
] as const

/** Read ELS+ enhancement level from workshop keys (canonical + import aliases). */
export function computeElsEnhancementLevel(
  ...
  for (const alias of ELS_ENHANCEMENT_ALIASES) {
...
/** Resolve ELS calculator defaults from a stored workshop blob when present. */
export function findElsLeadFromWorkshopBlob(blob: Record<string, unknown> | null | undefined): ElsWorkshopLead | null {
