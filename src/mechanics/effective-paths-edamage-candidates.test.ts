...
describe('the eDamage candidate lists', () => {
  for (const variant of variants) {
    it(`matches the sheet's ${variant} matrix, name for name and in order`, () => {
      ...
      expect(EFFECTIVE_DAMAGE_CANDIDATES[variant]).toEqual(fromSheet)
    })
  }

  it('offers 33, 29, 25 and 11', () => {
    expect(EFFECTIVE_DAMAGE_CANDIDATES.lab).toHaveLength(32)
    expect(EFFECTIVE_DAMAGE_CANDIDATES.stone).toHaveLength(29)
    expect(EFFECTIVE_DAMAGE_CANDIDATES.coin).toHaveLength(25)
    expect(EFFECTIVE_DAMAGE_CANDIDATES.keys).toHaveLength(11)
  ...
  it('leaves the Generator substat column blank on the lab path', () => {
    ...
    for (const variant of variants) {
      expect(
        EFFECTIVE_DAMAGE_CANDIDATES[variant].map(entry => entry.sheetName),
        variant,
  ...
  it('gives the stone path the ultimate weapons, which no other path buys', () => {
    ...
    const stone = EFFECTIVE_DAMAGE_CANDIDATES.stone.map(entry => entry.sheetName)
    for (const weapon of ['DW', 'CL', 'SM', 'SL', 'PS', 'ILM', 'CF']) {
      expect(stone.some(name => name.startsWith(`${weapon} `)), weapon).toBe(true)
    }
    const others = [...EFFECTIVE_DAMAGE_CANDIDATES.lab, ...EFFECTIVE_DAMAGE_CANDIDATES.coin]
  ...
  it('keeps every name distinct within a path', () => {
    for (const variant of variants) {
      const names = EFFECTIVE_DAMAGE_CANDIDATES[variant].map(entry => entry.sheetName)
