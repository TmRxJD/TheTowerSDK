...
describe('every candidate names a real lab', () => {
  for (const [key, name] of Object.entries(DISCOUNT_LAB_NAMES)) {
    it(`prices ${key}`, () => {
  ...
  it('gates five of the six on the priority selector', () => {
    ...
    expect([...DISCOUNT_PRIORITY_ORDER].sort()).toEqual(
      (Object.keys(DISCOUNT_LAB_NAMES) as EffectiveEconomyDiscountKey[])
...
describe('the priority selector', () => {
  ...
  it('skips a category with nothing left to buy', () => {
    const maxed = {
      ...ZERO,
      labsCoinDiscount: labMaxCatalogLevel(DISCOUNT_LAB_NAMES.labsCoinDiscount),
    }
...
describe('planning the path', () => {
  ...
  it('stops when every candidate is maxed rather than planning empty steps', () => {
    const maxed = Object.fromEntries(
      Object.entries(DISCOUNT_LAB_NAMES).map(([key, name]) => [key, labMaxCatalogLevel(name)]),
    ) as typeof ZERO
...
describe('the totals the priority selector weighs', () => {
  ...
  it('leaves out the four labs the sheet does not count', () => {
    // They are the only four with no row in `DVT_Laboratory_Unlock`, and at
...
describe('the retroactive discount labs, hidden by default', () => {
  ...
  it('says why the other five are off the path', () => {
    ...
    for (const key of RETROACTIVE_DISCOUNT_LABS) {
      expect(reasons.get(DISCOUNT_LAB_NAMES[key]), key).toMatch(/already spent/)
    }
    expect(reasons.has(DISCOUNT_LAB_NAMES.labsCoinDiscount)).toBe(false)
  })
