 ...
 */
/* ── file map ─────────────────────────────────────────────────────────────────
 * exports    botTowerFactor, botEffectiveRadius, getBotBenefit, flameBotDamageReductionMultiplier, FlameBotReductionInput, getFlameBotDamageReduction, ThunderBotLingerInput, getThunderBotLinger (+3 more)
 * depends    src/mechanics/primitives/constants, src/mechanics/primitives/math
 * used by    src/mechanics/bots/geometry, src/mechanics/bots/index, src/mechanics/calculators/specs-generated, src/mechanics/combat/damage-reduction, src/mechanics/combat/damage-redux-layers
...
import { clamp, inverseLerp, lerp } from '../primitives/math'
...
/**
 * Effective bot radius.
 *
 * The tower's contribution is `botTowerFactor`, not `maxDistance / 60`. The division is the
 * intuitive guess and it is wrong at every distance except by coincidence — it has no floor, where
 * the real curve starts at `BOT_TOWER_FACTOR_BASE` and rises across a bounded range, so a low
 * tower range under-reports the radius rather than scaling it.
 */
