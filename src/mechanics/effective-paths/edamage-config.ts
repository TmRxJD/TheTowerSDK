 ...
 */
/* ── file map ─────────────────────────────────────────────────────────────────
 * exports    DAMAGE_WORKSHOP_STATS, DamageWorkshopStat, DamageStatSource, ZERO_DAMAGE_STAT_SOURCE, MODULE_SLOTS, ModuleSlot, DamageModuleSource, ZERO_DAMAGE_MODULE_SOURCE (+16 more)
 * depends    src/mechanics/effective-paths/damage-base, src/mechanics/effective-paths/damage-substats
 * used by    src/mechanics/calculators/specs-generated, src/mechanics/effective-paths/edamage-compute, src/mechanics/effective-paths/edamage-plan, src/mechanics/effective-paths/edamage-schema, src/mechanics/effective-paths/index
 * ─────────────────────────────────────────────── generated by `npm run map` ──
 */

import type { DamageRunType } from './damage-base'
import type { SubstatPair } from './damage-substats'
 ...
 */
export const DAMAGE_SUBSTAT_NAMES = [
...
export type DamageSubstat = typeof DAMAGE_SUBSTAT_NAMES[number]
...
export interface DamageUltimateWeaponSource {
  ...
  plus: number | null
  /**
   * `BM` on the weapon's own row — vault node levels as a fraction (`BN = BM*5%`).
   *
   * Distinct from {@link EffectiveDamageConfig.ultimateWeaponDamageVaultPct}
   * (`BM39`), which is the shared UW Damage vault. Spotlight's
   * `STAT_UW_SL_FINAL_DMG` multiplies both. Optional — absent means zero.
   */
  vaultPct?: number
}
...
export interface EffectiveDamageConfig {
  ...
  /**
   * `CY7` — how long a wave lasts, which Amplifying Strike is spread over and
  ...
  cash: number
  /**
   * `AY27` — Show Labs on Coin path. First word `None` hides every
   * non-enhancement coin candidate (labs, modules, dissonant echoes).
   */
  showLabsOnCoinPath: boolean
  /**
   * `AY26` — Hide Non-unlocked Labs.
   *
   * When true, each lab ROI column's hide row ANDs
   * `NOT(IDS_LAB_HAS_UNLOCKED(…))`. Offering a locked lab still scores a
   * gain (often from a free level-0 row), so the path ranks it while the
   * sheet blanks the column — sheet-2 Echo vs Super Crit Multi.
   */
  hideNonUnlockedLabs: boolean
  /**
   * `eDamage!AL4` — cannon module level horizon label for
   * `EPG_MODULE_LEVEL_LIMIT`. Coin ROI blanks Primary/Assist Module - Cannon
   * when `level$5 + limit <= level`.
   *
   * `undefined` means the cell was not captured — do not invent a limit.
   * `""` / `"None …"` are real sheet values (limit 0).
   */
  cannonModuleLevelHorizon?: string
  /**
   * `eDamage!AL23` — same horizon for Primary/Assist Module - Core.
   */
  coreModuleLevelHorizon?: string
  /**
...
export function zeroEffectiveDamageConfig(): EffectiveDamageConfig {
  ...
  return {
    ...
    cash: 0,
    showLabsOnCoinPath: true,
    hideNonUnlockedLabs: false,
    ...
    substats: fromKeys(DAMAGE_SUBSTAT_NAMES, () => ({ primary: 0, assist: 0 })),
