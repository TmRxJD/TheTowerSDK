/* ── file map ─────────────────────────────────────────────────────────────────
 * exports    InputRangeKind, InputRange, EP_PATH_ROW_CAP, NOT_LABS, isDamageLabUpgrade, SHOW_LEVELS_OPTIONS, ShowLevelsOption, showLevelsCap (+5 more)
 * depends    src/data/bots/data, src/data/cards/data, src/data/guardians/upgrades, src/data/modules/levels, src/data/vault/tree, src/data/workshop/table (+5 more)
...
import { CARD_TEMPLATE_MAP } from '../../data/cards/data'
import { guardianUpgrades } from '../../data/guardians/upgrades'
import { MODULE_RARITY_LEVEL_CAPS } from '../../data/modules/levels'
...
import { resolveUltimateWeaponStat, ultimateWeaponMaxLevel } from './edamage-costs'
import { labMaxCatalogLevel } from './lab-costs'
import { EFFECTIVE_DAMAGE_UPGRADES } from './edamage-plan'
import { EFFECTIVE_ECONOMY_UPGRADES } from './eecon-plan'
import { EFFECTIVE_HEALTH_UPGRADES } from './ehp-plan'
 ...
 * So the maximum is the ceiling the stat itself declares, per bot and per stat,
 * which is the only place the truth is. It used to be read as the highest key of
 * an enumerated level table; the stat now carries `maxLevel` directly.
 */
  ...
  for (const bot of BOT_UPGRADES_DATA) {
    for (const [stat, definition] of Object.entries(bot.stats ?? {})) {
      ranges.push({
        ...
        kind: 'level',
        source: 'BOT_UPGRADES_DATA — the ceiling the stat itself declares',
        min: 0,
        max: definition?.maxLevel ?? 0,
      })
    }
  }
...
function guardianRanges(): InputRange[] {
   ...
   * `Recovery`/`Max Recovery` where the catalog says `Recovery Amount` — so the
