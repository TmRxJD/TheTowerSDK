 ...
 */
/* ── file map ─────────────────────────────────────────────────────────────────
 * exports    ULTIMATE_WEAPON_ABBREVIATIONS, ultimateWeaponStats, UltimateWeaponStat, resolveUltimateWeaponStat, ultimateWeaponStoneCost, UW_PLUS_UNLOCK_STONE_COSTS_BY_OWNED, uwPlusUnlockStoneCost, ultimateWeaponMaxLevel (+7 more)
 * depends    src/data/ultimate-weapons/stones, src/data/vault/tree
 * used by    src/mechanics/calculators/run, src/mechanics/calculators/specs-generated, src/mechanics/effective-paths/edamage-compute, src/mechanics/effective-paths/edamage-plan, src/mechanics/effective-paths/eecon-plan, src/mechanics/effective-paths/eecon-uw-cd (+2 more)
 ...
 *
 * Level 0 on a base UW stat is the weapon's unlock, which the chart prices as
 * the string `"Unlock"` rather than a number — so it returns `null` rather than
 * pretending a weapon can be bought from this ladder.
 *
 * Level 0 on a UW+ (`Cover Fire`, `Light Range`, …) is also `"Unlock"` on the
 * chart. The sheet prices that unlock with `EP_NEXT_UWP_COST_OFFSET` /
 * `All UWs!E6:E15` — see {@link uwPlusUnlockStoneCost}. Callers that plan the
 * plus band must use that when this returns `null` at level 0.
 */
...
/**
 * `All UWs!E6:E15` — stones to unlock the next UW+, indexed by how many pluses
 * the account already owns (`EP_NEXT_UWP_COST_OFFSET(0)` when `ownedCount` is
 * the live count of non-`"Lo | Locked"` plus cells).
 *
 * Matches `uwPlusUnlockCostsByOwnedCount` on the charts surface; kept here so
 * mechanics never imports charts.
 */
export const UW_PLUS_UNLOCK_STONE_COSTS_BY_OWNED: readonly number[] = [
  500, 625, 750, 975, 1250, 1650, 2200, 2900, 3800,
]

/** Stones to unlock one more UW+ when `ownedPlusCount` pluses are already owned. */
export function uwPlusUnlockStoneCost(ownedPlusCount: number): number | null {
  if (!Number.isInteger(ownedPlusCount) || ownedPlusCount < 0) return null
  const cost = UW_PLUS_UNLOCK_STONE_COSTS_BY_OWNED[ownedPlusCount]
 ...
 * `"300s"`, `"20%"` — because that is what a player is shown. The model
