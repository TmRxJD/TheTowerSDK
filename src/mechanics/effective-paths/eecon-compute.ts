 ...
 */
/* ── file map ─────────────────────────────────────────────────────────────────
 * exports    EconomySubstat, COMPARE_MASTERIES_AT_DEFAULT, compareMasteriesAtDigit, compareMasteriesAtFromLabLevels, EconomyCard, EconomyWorkshopStat, EffectiveEconomyConfig, EffectiveEconomyBreakdown (+6 more)
 * depends    src/data/modules/bonus, src/mechanics/effective-paths/eecon-levels, src/mechanics/effective-paths/eecon-stats, src/mechanics/effective-paths/ehp-model, src/mechanics/effective-paths/generics
 * used by    src/mechanics/calculators/specs-generated, src/mechanics/effective-paths/eecon-plan, src/mechanics/effective-paths/eecon-stone-mastery, src/mechanics/effective-paths/eecon-uw-cd, src/mechanics/effective-paths/index, src/save/effective-paths/from-save
 * ─────────────────────────────────────────────── generated by `npm run map` ──
 */

import { computeModuleStat } from '../../data/modules/bonus'
import { dissonantBoostOfType } from './ehp-model'
import { moduleBonus } from './generics'
import {
  blackHoleCoinBonus,
  blackHoleCooldown,
  blackHoleDuration,
 ...
 *
 * On the sheet AX29 is a formula (`ROUND(SUMIF(*Mastery*)/COUNTIF(…))`), not an
 * editable input. Callers store the digit {@link compareMasteriesAtDigit} so
 * planner math matches `RIGHT(AX29,1)` without re-applying it.
 */
export const COMPARE_MASTERIES_AT_DEFAULT = 0

/**
 * Digits the sheet's `RIGHT(AX29, 1)` would return for a rounded AX29 value.
 */
export function compareMasteriesAtDigit(ax29: number): number {
  if (!Number.isFinite(ax29)) return COMPARE_MASTERIES_AT_DEFAULT
  return Math.abs(Math.trunc(ax29)) % 10
}

/**
 * Sheet `eEcon!AX29` average: round(sum of mastery lab levels / count), then
 * the last digit — what stone mastery ROI reads.
 *
 * Empty list → {@link COMPARE_MASTERIES_AT_DEFAULT} (same as `IFERROR(…, 0)`).
 */
export function compareMasteriesAtFromLabLevels(
  masteryLabLevels: readonly number[],
): number {
  const levels = masteryLabLevels.filter(level => Number.isFinite(level) && level >= 0)
  if (levels.length === 0) return COMPARE_MASTERIES_AT_DEFAULT
  const ax29 = Math.round(levels.reduce((sum, level) => sum + level, 0) / levels.length)
  return compareMasteriesAtDigit(ax29)
}
 ...
 *
 * These are inputs nothing else holds, and they are **not** zero on the sheet:
