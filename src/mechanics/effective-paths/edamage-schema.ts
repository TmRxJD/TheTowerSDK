 ...
 * what arrives at runtime from a player's targets — a `NaN` where a level
 ...
 */
/* ── file map ─────────────────────────────────────────────────────────────────
 * exports    effectiveDamageConfigSchema, effectiveDamageLevelsSchema, EffectiveDamageInputIssue, EffectiveDamageInputCheck, checkEffectiveDamageInputs, assertEffectiveDamageInputs
 * depends    src/mechanics/effective-paths/edamage-config, src/mechanics/effective-paths/edamage-levels
...
import type { EffectiveDamageConfig } from './edamage-config'
...
const weaponSourceSchema = z.object({
  ...
  /** `BM` on the weapon row as a fraction (`BN = BM*5%`). */
  vaultPct: finite.default(0),
})
...
export const effectiveDamageConfigSchema = z.object({
  ...
  substats: exactRecord(DAMAGE_SUBSTAT_NAMES, substatPairSchema),
 ...
 * worse than a page that says which of your targets is not readable, and the
