 ...
 */
/* ── file map ─────────────────────────────────────────────────────────────────
 * exports    workshopEnhancementLevelFromMap, WorkshopEnhancementCategory, WORKSHOP_ENHANCEMENT_CATEGORIES, WorkshopEnhancementDiscounts, enhancementCoinCost, enhancementMaxLevel, enhancementStats, EFFECTIVE_HEALTH_ENHANCEMENTS (+8 more)
 * depends    src/data/workshop/enhancement-costs.json, src/data/workshop/enhancement-progress-definitions
...
/**
 * Workshop codes (`WSP_CASH_BONUS`) → sheet display names (`Cash Bonus`).
 *
 * Saves and imports key by code; every Effective Paths formula and cost
...
/**
 * An enhancement's level from a map that may use display names, `WSP_*` codes,
 * or both.
 */
export function workshopEnhancementLevelFromMap(
  ...
  // Cost tables say `Damage/Meter`; the display label is `Damage / Meter`.
 ...
 * function at five level sets, exactly. Accepts `WSP_*` codes as well
