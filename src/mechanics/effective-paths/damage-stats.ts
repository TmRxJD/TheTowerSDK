 ...
 */
/* ── file map ─────────────────────────────────────────────────────────────────
 * exports    CriticalChanceInput, criticalChance, CriticalInput, criticalMultiplier, ultimateWeaponCriticalMultiplier, multishotChance, multishotTargets, multishotMultiplier (+24 more)
 * depends    —
 * used by    src/mechanics/calculators/run, src/mechanics/calculators/specs-generated, src/mechanics/effective-paths/edamage-compute, src/mechanics/effective-paths/index, src/mechanics/effective-paths/uw-dps
 ...
 *
 * Additive vault, from the canonical Effective Paths dump
 * `temp/EffectivePaths-canonical.xlsx` (v5.10.02):
 *
 *     EPD_MSC: LAMBDA(ws_level, substat, vault,
 *       LET(WS, ws_level * 0.5%, WS + Substat + vault))
 *
 * The multiplicative `(WS + Substat) * (1 + vault)` form matched a stale
 * working-copy extract (v5.09.03.07, ~8/19) and is wrong against v5.10.02.
 */
export function multishotChance(workshopLevel: number, substat: number, vault: number): number {
  return workshopLevel * 0.005 + substat + vault
}
 ...
 *
 * Additive vault, capped at 1, from the canonical dump
 * `temp/EffectivePaths-canonical.xlsx` (v5.10.02):
 *
 *     EPD_BSC: LAMBDA(ws_level, substat, vault,
 *       LET(WS, ws_level * 0.8%, MIN(1, WS + Substat + vault)))
 *
 * `MIN(1, …)` arrived in v5.10.02.00. Same stale WC multiplicative form as
 * {@link multishotChance} is wrong against this dump.
 */
export function bounceShotChance(workshopLevel: number, substat: number, vault: number): number {
  return Math.min(1, workshopLevel * 0.008 + substat + vault)
}
 ...
 *
 * Additive vault, from the canonical dump
 * `temp/EffectivePaths-canonical.xlsx` (v5.10.02):
 *
 *     EPD_RFC: LAMBDA(ws_level, substat, vault,
 *       LET(WS, ws_level * 0.4%, WS + Substat + vault))
 *
 * Same stale WC multiplicative form as {@link multishotChance} is wrong
 * against this dump.
 */
export function rapidFireChance(workshopLevel: number, substat: number, vault: number): number {
  return workshopLevel * 0.004 + substat + vault
}
