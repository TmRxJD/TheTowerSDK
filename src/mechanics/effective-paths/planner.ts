 ...
 * depends    —
 * used by    src/mechanics/calculators/specs-generated, src/mechanics/effective-paths/edamage-plan, src/mechanics/effective-paths/eecon-discount, src/mechanics/effective-paths/eecon-plan, src/mechanics/effective-paths/ehp-plan, src/mechanics/effective-paths/index (+1 more)
...
export interface PathPlanOptions {
   ...
   *
   * `current` is the running level map for this step (before the candidate is
   * applied). UW+ unlock pricing needs it: `EP_NEXT_UWP_COST_OFFSET` climbs as
   * the path buys pluses, so a cost closed over the starting levels alone
   * undercharges every unlock after the first.
   *
   ...
   * "not a candidate".
   */
  cost: (id: string, nextLevel: number, current: ReadonlyMap<string, number>) => number
  /**
   ...
   */
  relativeGain?: (id: string, nextLevel: number, levels: ReadonlyMap<string, number>) => number | null
  /**
   * Later candidates must beat the incumbent by more than this relative margin.
   *
   * Exact ties already keep the leftmost column (`roi > best.roi`). A small
   * soft margin matches the sheet when a right-hand column is only barely
   * ahead due to float / residual shadow error — e.g. Max Rend vs Critical
   * Factor (~0.5% port, sheet prefers CF). `0` preserves strict comparison.
   */
  roiSoftTieRelative?: number
  /**
...
export function planPath(options: PathPlanOptions): PathStep[] {
  ...
  for (let step = 1; step <= steps; step++) {
    ...
    for (const upgrade of upgrades) {
      ...
      const price = cost(upgrade.id, nextLevel, levels)
      ...
      const relative = relativeGain?.(upgrade.id, nextLevel, levels) ?? null
      ...
      // Strictly greater (plus optional soft margin), so the earliest upgrade
      // wins a tie — the sheet takes the leftmost column of the joint maximum.
      const beat = best === null
        || roi > best.roi * (1 + roiSoftTieRelative)
      if (beat) {
