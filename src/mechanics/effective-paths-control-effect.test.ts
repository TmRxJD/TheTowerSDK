...
const CONFIG_SOURCE = readFileSync(
  fileURLToPath(new URL('./effective-paths-edamage-config.ts', import.meta.url)),
  'utf8',
)
...
const PATH_ONLY_CONTROLS: Record<string, string> = {
  ...
  AY26: 'Hide Non-unlocked Labs — row-2 hide flags on every column',
  AY27: 'Show Labs on Coin path — a level HORIZON via EPG_MODULE_LEVEL_LIMIT',
...
describe('every exposed control changes the answer', () => {
  ...
  it('sweeps every top-level boolean the config has', () => {
    ...
    expect(booleans.sort()).toEqual([
      'cardsEquipped', 'chronoFieldEnabled', 'hasRendArmour', 'perksEquipped',
      'shockMultiplierUnlocked',
    ])
  })
