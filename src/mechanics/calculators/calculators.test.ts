...
import { calculatorGraph, calculatorsProducing, calculatorsReading, upstreamOf } from './graph'
import { getSharedToolLabs } from '../../data/labs'
...
const SAMPLES: Record<string, { args: Record<string, unknown>, moved: Record<string, unknown> }> = {
  ...
  'inputs.mergeNumberRecords': {
    args: { sources: [{ a: 1 }, { b: 2 }] },
    moved: { sources: [{ a: 1 }, { a: 9 }] },
  },
  'inputs.mergeNumberArrayRecords': {
    args: { sources: [{ a: [1] }, { b: [2] }] },
    moved: { sources: [{ a: [1] }, { a: [9] }] },
  },
  'inputs.mergeNestedNumberRecords': {
    args: { sources: [{ g: { a: 1 } }, { g: { b: 2 } }] },
    moved: { sources: [{ g: { a: 1 } }, { g: { a: 9 } }] },
  },
 ...
 * `lab.trackerMaxLevel` is that it falls back to the research catalog when the
  ...
  it('points every declaration at a module that exists and exports the symbol', () => {
    ...
    for (const spec of CALCULATORS) {
      ...
      const exported = new RegExp(
        `export\\s+(?:async\\s+)?(?:function\\s+${spec.symbol}\\b|const\\s+${spec.symbol}\\s*[:=])`,
      ).test(text)
