...
describe('the three matrices', () => {
  for (const [variant, list, count] of ALL) {
    it(`has the ${variant} path's ${count}`, () => {
      ...
      expect(EFFECTIVE_ECONOMY_CANDIDATES[variant]).toBe(list)
    })
...
describe('what the three paths actually optimise', () => {
  it('does not pretend the discount path is comparable to the other two', () => {
    ...
    expect(EFFECTIVE_ECONOMY_OUTPUT_CELLS.time.metric)
      .toBe(EFFECTIVE_ECONOMY_OUTPUT_CELLS.stone.metric)
    expect(EFFECTIVE_ECONOMY_OUTPUT_CELLS.discount.metric).toBe('coins saved')
  })

  it('records where each tab writes its answer', () => {
    for (const [variant] of ALL) {
      const output = EFFECTIVE_ECONOMY_OUTPUT_CELLS[variant]
