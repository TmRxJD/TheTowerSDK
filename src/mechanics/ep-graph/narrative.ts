/* ── file map ─────────────────────────────────────────────────────────────────
 * exports    renderEpRelationsNarrative
...
export function renderEpRelationsNarrative(graph: EpGraph): string {
  const lines = [
    ...
    '',
    '## Controls → consumers (feeds)',
    '',
    '> Explicit reverse of `reads` into a `control` node. Do not invent a second graph.',
    '',
    ...orEmpty(bulletsForKind(graph, 'feeds')),
    '',
    '## Depends on (configuration prerequisites)',
    '',
    ...orEmpty(bulletsForKind(graph, 'dependsOn')),
    '',
    '## Affects path (control → planner path)',
    '',
    ...orEmpty(bulletsForKind(graph, 'affectsPath')),
    '',
