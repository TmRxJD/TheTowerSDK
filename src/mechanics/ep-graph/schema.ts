/* ── file map ─────────────────────────────────────────────────────────────────
 * exports    EpGraphFamilySchema, EpGraphFamily, EpGraphNodeTypeSchema, EpGraphNodeType, EpSourceCellKindSchema, EpSourceCellKind, EpSourceCellSchema, EpSourceCell (+22 more)
 * depends    —
 * used by    src/mechanics/calculators/specs-generated, src/mechanics/ep-graph/build-index, src/mechanics/ep-graph/index, src/mechanics/ep-graph/mutate, src/mechanics/ep-graph/narrative, src/mechanics/ep-graph/render-mermaid (+3 more)
...
export const EpEdgeKindSchema = z.enum([
  'reads',
  /**
   * Reverse of `reads` when the dependency target is a control: control →
   * consumer. Lets ControlRelation queries walk outward from panel leaves
   * without flipping the historical `reads` direction in place.
   */
  'feeds',
  /**
   * Prerequisite walk: dependent → required node (e.g. presetsMaster → runType).
   * Distinct from `reads` (formula consumption) — this is configuration order.
   */
  'dependsOn',
  /**
   * Control → path hub: which planner path(s) / variants the control moves.
   * Optional `pathVariants` on the edge names the sub-tabs.
   */
  'affectsPath',
  'hides',
  'gates',
...
export const EpGraphEdgeBaseSchema = z.object({
  ...
  /**
   * Sub-path variants this edge justifies (affectsPath). Sheet language:
   * lab / coin / stone / keys / discount / time — not invent new labels.
   */
  pathVariants: z.array(z.string().min(1)).optional(),
})
